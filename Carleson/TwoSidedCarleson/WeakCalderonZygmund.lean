...
private theorem maximal_theorem'' (hα : α > 0) (hf : BoundedFiniteSupport f) :
...
private lemma α'_pos {α : ℝ≥0∞} (hα : α > 0) : α' a α > 0 :=
...
private lemma lemma_10_2_7_bound' (hx : x ∈ (Ω f (α' a α))ᶜ) (hX : GeneralCase f (α' a α)) (j : ℕ)
  ...
  _ ≤ ∫⁻ y in czBall3 hX j, (edist (czCenter hX j) y / edist x (czCenter hX j)) ^ (a : ℝ)⁻¹ *
    ...
    rw [enorm_sub_rev]
    simp only [Ω, hX, reduceDIte, compl_iUnion, mem_iInter, mem_compl_iff, mem_ball, not_lt] at hx
    exact enorm_K_sub_le <| calc 2 * dist (czCenter hX j) y
      ...
      _ ≤ dist x (czCenter hX j)  := le_of_eq_of_le (by ring) (hx j)
  ...
  _ = _ := lintegral_const_mul'' _ g_aemeas.enorm.restrict
  ...
  _ = _ := by rw [C10_2_7_def, add_assoc, ← pow_add]; norm_cast

-- Bound used repeatedly in the proof of Lemma 10.2.7
private lemma lemma_10_2_7_bound (hx : x ∈ (Ω f (α' a α))ᶜ) (hX : GeneralCase f (α' a α)) (j : ℕ)
    (hr : 0 < r) {g : X → ℂ} (g_int : Integrable g) (hg0 : ∫ y in czBall3 hX j, g y = 0)
    (hg : ∫⁻ y in czBall3 hX j, ‖g y‖ₑ ≤ 2 ^ (2 * a + 1) * (α' a α) * volume (czBall3 hX j)) :
    ‖∫ y in czBall3 hX j, K x y * g y‖ₑ ≤
    C10_2_7 a * α * (((3 * czRadius hX j).toNNReal / nndist x (czCenter hX j)) ^ (a : ℝ)⁻¹ *
    volume (czBall3 hX j)) / volume (ball x (dist x (czCenter hX j))) := calc
  _ = ‖∫ y in czBall3 hX j, (K x y - K x (czCenter hX j)) * g y‖ₑ := by
    ...
    · sorry
    · sorry
/-       refine (integrableOn_K_mul g_int x hr).mono_set ?_
      intro y hy hy'
...
private lemma 𝒥₁_bound {r : ℝ} (hr : 0 < r) (hα : 0 < α) {x : X} (hx : x ∈ (Ω f (α' a α))ᶜ)
  ...
  _ = ‖∫ y in czBall3 hX j, (K x y - K x (czCenter hX j)) * (czRemainder' hX j y)‖ₑ := by
    apply congrArg
    simp_rw [sub_mul]
    rw [integral_sub]
    · symm
    ...
    · apply (integrableOn_K_mul (hf.czRemainder' (α'_pos hα) hX j).integrable x hr).mono_set
      intro y hy hy'
      simp only [mem_setOf_eq, mem_ball, 𝒥₁] at hj hy hy'
      linarith [dist_triangle_left x (czCenter hX j) y]
  ...
  _ ≤ _ := by
    apply lemma_10_2_7_bound' hx hX j (hf.aemeasurable.czRemainder' hX j)
...
private lemma tsum_𝒥₃ (r : ℝ) (x : X) (hX : GeneralCase f (α' a α)) :
  ...
  refine enorm_eq_zero.mpr (setIntegral_eq_zero_of_forall_eq_zero (fun y hy ↦ ?_))
...
omit [IsTwoSidedKernel a K] in
private lemma czOperator_czRemainder' (hX : GeneralCase f α) (x : X) (i : ℕ) :
  ...
  simp_rw [← indicator_mul_right, setIntegral_indicator (MeasurableSet.czPartition hX i)]
  apply setIntegral_congr_fun <| measurableSet_ball.compl.inter (MeasurableSet.czPartition hX i)
  intro y hy
  simp [czRemainder]

private lemma czOperator_czRemainder (hr : 0 < r) (hα : α > 0) (hX : GeneralCase f α)
  ...
  · apply Integrable.bdd_mul' (c := (C_K a / volume (ball x r)).toReal)
    · exact ⟨(AEMeasurable.czRemainder hα (hf := hf)).aestronglyMeasurable.restrict,
        (hf.czRemainder hα).integrable.hasFiniteIntegral.mono_measure Measure.restrict_le_self⟩
    · exact (measurable_K_right x).aestronglyMeasurable.restrict
    · refine eventually_of_mem (self_mem_ae_restrict (by measurability)) (fun y hy ↦ ?_)
      obtain ⟨_, ⟨⟨j, rfl⟩, hj⟩⟩ := hy
      have := enorm_K_le_ball_complement (K := K) hj.1
      rw [← toReal_enorm, ENNReal.toReal_le_toReal enorm_ne_top]
...
lemma estimate_bad_partial {hf : BoundedFiniteSupport f} (hr : 0 < r)
  ...
  _ = ‖∑' j, czOperator K r _ x‖ₑ := by rw [czOperator_czRemainder hr (α'_pos (pos_of_gt hα)) hX hf]
  ...
  _ ≤ _ + _ := (ENNReal.tsum_union_le (fun i ↦ ‖czOperator K r (czRemainder' hX i) x‖ₑ) _ _)
  _ ≤ _ + (2 * _ + α / 8) := add_le_add (tsum_𝒥₁ hf hr (pos_of_gt hα) hX hx) (tsum_𝒥₂ r hX hx)
