...
theorem classical_carleson_Lp {f : ℝ → ℂ} (periodic_f : f.Periodic (2 * π))
  {p : ℝ≥0∞} (hp : 1 < p) (hf : MemLp f p (volume.restrict (Set.Ioc 0 (2 * π)))) :
...
theorem classical_carleson_from_Lp : ClassicalCarleson := by
  unfold ClassicalCarleson
  intro f continuous_f periodic_f
  apply classical_carleson_Lp (p := ⊤) periodic_f (by simp)
