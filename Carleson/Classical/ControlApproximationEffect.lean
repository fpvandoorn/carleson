...
lemma rcarleson_exceptional_set_estimate {δ C : NNReal} (Cpos : 0 < C) {f : ℝ → ℂ}
  ...
  calc δ * volume E
    ...
    _ ≤ C * (C10_0_1 4 2 * (volume E) ^ (2 : ℝ)⁻¹ * (volume F) ^ (2 : ℝ)⁻¹) := by
      ...
      intro x
      simp only [one_div, Complex.norm_mul, norm_inv, norm_real, Real.norm_eq_abs, NNReal.abs_eq]
...
--TODO: can be removed once `two_sided_metric_carleson_hasStrongType` no longer depends on `sorryAx`
lemma rcarleson_exceptional_set_estimate_specific {δ C : NNReal} (Cpos : 0 < C) {f : ℝ → ℂ}
...
--TODO: can be removed once `two_sided_metric_carleson_hasStrongType` no longer depends on `sorryAx`
/-- The constant used in `C_distribution_carlesonOperatorReal_le'`. -/
def C_distribution_carlesonOperatorReal_le' (δ ε : NNReal) : NNReal :=
  (ε / ((C10_0_1 4 2) ^ (2 : ℝ) * Real.toNNReal (2 * π + 2) / δ ^ (2 : ℝ))) ^ (2 : ℝ)⁻¹

--TODO: can be removed once `two_sided_metric_carleson_hasStrongType` no longer depends on `sorryAx`
lemma C_distribution_carlesonOperatorReal_le'_pos {δ ε : NNReal} (δpos : 0 < δ) (εpos : 0 < ε) :
...
--TODO: can be removed once `two_sided_metric_carleson_hasStrongType` no longer depends on `sorryAx`
lemma distribution_carlesonOperatorReal_le' {δ ε : NNReal} (δpos : 0 < δ) (εpos : 0 < ε) {g : ℝ → ℂ}
...
--TODO: can be removed once `two_sided_metric_carleson_hasStrongType` no longer depends on `sorryAx`
/-- The constant used in `C_control_approximation_effect'`. -/
def C_control_approximation_effect' (δ ε : NNReal) : NNReal :=
  min (δ / Real.toNNReal (2 * π))
    (C_distribution_carlesonOperatorReal_le' (2 * π * (↑δ / 2) / 2).toNNReal (ε / 2))

--TODO: can be removed once `two_sided_metric_carleson_hasStrongType` no longer depends on `sorryAx`
lemma C_control_approximation_effect'_pos {δ ε : NNReal} (δpos : 0 < δ) (εpos : 0 < ε) :
    0 < C_control_approximation_effect' δ ε :=
  lt_min (by positivity)
    (C_distribution_carlesonOperatorReal_le'_pos (by positivity) (by positivity))

--TODO: can be removed once `two_sided_metric_carleson_hasStrongType` no longer depends on `sorryAx`
lemma C_control_approximation_effect'_property {δ ε : NNReal} :
...
--TODO: can be removed once `two_sided_metric_carleson_hasStrongType` no longer depends on `sorryAx`
lemma C_control_approximation_effect'_le {δ ε : NNReal} :
...
--TODO: can be removed once `two_sided_metric_carleson_hasStrongType` no longer depends on `sorryAx`
/- This is Lemma 11.6.4 (partial Fourier sums of small) in the blueprint.-/
lemma control_approximation_effect' {δ ε : NNReal} (δpos : 0 < δ) (εpos : 0 < ε)
