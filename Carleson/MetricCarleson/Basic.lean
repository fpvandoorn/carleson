...
@[simp]
theorem carlesonOperator_zero [Countable (Θ X)] :
    carlesonOperator K 0 = 0 := by
  unfold carlesonOperator linearizedCarlesonOperator carlesonOperatorIntegrand
  simp
  rfl

theorem carlesonOperatorIntegrand_congr_ae (h : f =ᶠ[ae volume] g) [Countable (Θ X)]
  (x : X) (θ : Θ X) (R₁ R₂ : ℝ) :
...
theorem linearizedCarlesonOperator_congr_ae (h : f =ᶠ[ae volume] g) [Countable (Θ X)]
  (x : X) (θ : Θ X) :
...
theorem carlesonOperator_congr_ae (h : f =ᶠ[ae volume] g) [Countable (Θ X)] :
...
theorem carlesonOperator_zero_of_ae_zero (hf : f =ᶠ[ae volume] 0) [Countable (Θ X)] :
