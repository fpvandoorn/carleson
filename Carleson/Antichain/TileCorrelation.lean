...
def C6_2_1 (a : ℕ) : ℝ≥0 := 2 ^ ((2 * CDN + 6 + CDN/4) * a ^ 3)
...
lemma e625 {s₁ s₂ : ℤ} {x₁ x₂ y y' : X} (hy' : y ≠ y') (hs : s₁ ≤ s₂) :
    ...
    2 ^ ((2 * CDN + 5 + CDN/4) * a ^ 3) / (volume (ball x₁ (D ^ s₁))
  ...
  calc
    ...
    _ ≤ 2 ^ ((2 * CDN + 4 + CDN/4) * a ^ 3) / (volume (ball x₁ (D ^ s₁)) *
        volume (ball x₂ (D ^ s₂))) *
        (edist y y' ^ τ / (D ^ s₁) ^ τ + edist y y' ^ τ / (D ^ s₂) ^ τ) := by
      have h2 : (2 : ℝ≥0∞) ^ ((2 * CDN + 4 + CDN/4) * a ^ 3) = C2_1_3 a * D2_1_3 a := by
    ...
    _ ≤ 2 ^ ((2 * CDN + 5 + CDN/4) * a ^ 3) / (volume (ball x₁ (D ^ s₁)) *
      ...
      rw [show (2 : ℝ≥0∞) ^ (-τ) * 2 ^ ((2 * CDN + 5 + CDN/4) * a ^ 3) =
        2 ^ ((2 * CDN + 4 + CDN/4) * a ^ 3) * (2 ^ (a ^ 3) * 2 ^ (-τ)) by ring]; gcongr
...
lemma correlation_kernel_bound {s₁ s₂ : ℤ} {x₁ x₂ : X} (hs : s₁ ≤ s₂) :
  ...
  calc
    ...
    _ ≤ _ := by
      ...
      calc
        _ ≤ 2 ^ ((2 * CDN + 5 + CDN/4) * a ^ 3) + 2 ^ ((2 * CDN + 5 + CDN/4) * a ^ 3) := by
          rw [← pow_mul]
          gcongr 2 ^ ?_ + 2 ^ ?_
          · norm_num
          · calc
            _ = CDN * a ^ 3 * 2 + a ^ 3 * 4 + 0 := by ring
            _ ≤ CDN * a ^ 3 * 2 + a ^ 3 * 5 + a ^ 3 * (CDN / 4) := by
              gcongr
              · norm_num
              · positivity
            _ = _ := by ring
        ...
        _ ≤ _ := by
          rw [← two_mul, ← pow_succ', show 1 = 1 ^ 3 by norm_num,
            show (2 * CDN + 6 + CDN/4) * a ^ 3 = (2 * CDN + 5 + CDN/4) * a ^ 3 + a ^ 3 by ring]
          gcongr
          · exact one_le_two
...
def C6_1_5 (a : ℕ) : ℝ≥0 := 2 ^ ((2 * CDN + 7 + CDN/4) * a ^ 3)

-- TODO : 4 ≤ a in blueprint
lemma C6_1_5_bound (ha : 4 ≤ a) :
    2 ^ ((2 * CDN + 6 + CDN/4) * a ^ 3 + 1) * 2 ^ (11 * a) ≤ C6_1_5 a := by
...
lemma I12_le' (p p' : 𝔓 X) (hle : 𝔰 p' ≤ 𝔰 p) (g : X → ℂ) (x1 : E p') (x2 : E p) :
    I12 p p' g x1 x2 ≤ (2^((2 * CDN + 6 + CDN/4) * a^3 + 8 * a)) *
  ...
  have heq : (2^((2 * CDN + 6 + CDN/4) * a^3 + 8 * a)) *
      ((1 + edist_{(x1 : X), ((D : ℝ) ^ 𝔰 p')} (Q x1) (Q x2))^(-(2 * a^2 + a^3 : ℝ)⁻¹)) /
      (volume (ball (x2 : X) (D ^𝔰 p))) =
      (2^((2 * CDN + 6 + CDN/4) * a^3 + 8 * a)) / (volume (ball (x2 : X) (D ^𝔰 p))) *
...
lemma I12_nnreal_le' (p p' : 𝔓 X) (hle : 𝔰 p' ≤ 𝔰 p) (g : X → ℂ) (x1 : E p') (x2 : E p) :
    (I12 p p' g x1 x2).toNNReal ≤ (2^((2 * CDN + 6 + CDN/4) * a^3 + 8 * a)) *
  ...
  have heq : (2^((2 * CDN + 6 + CDN/4) * a^3 + 8 * a)) *
      ...
      (2^((2 * CDN + 6 + CDN/4) * a^3 + 8 * a)) / (volume (ball (x2 : X) (D ^𝔰 p))).toNNReal *
...
lemma I12_nnreal_le (ha : 1 < a) {p p' : 𝔓 X} (hle : 𝔰 p' ≤ 𝔰 p) (g : X → ℂ)
    ...
    (2^((2 * CDN + 6 + CDN/4) * a^3 + 8 * a + 1) * ((1 + dist_(p') (𝒬 p') (𝒬 p))^(-(2 * a^2 + a^3 : ℝ)⁻¹))) /
      (volume (ball (x2 : X) (D ^𝔰 p))).toNNReal * ‖g ↑x1‖₊ * ‖g ↑x2‖₊ := by
...
lemma bound_6_2_29 (ha : 4 ≤ a) (p p' : 𝔓 X) (x2 : E p) :
  ...
  have h4 : (2 : ℝ≥0∞) ^ ((2 * CDN + 6 + CDN/4) * a ^ 3 + 1) * 2 ^ (11 * a) ≤ C6_1_5 a :=
  ...
  calc 2 ^ ((2 * CDN + 6 + CDN/4) * a^3 + 8 * a + 1) *
    ((1 + nndist_(p') (𝒬 p') (𝒬 p))^(-(2 * a^2 + a^3 : ℝ)⁻¹)) /
      (volume (ball (x2 : X) (D ^𝔰 p)))
    _ = 2^ ((2 * CDN + 6 + CDN/4) * a^3 + 1) * 2 ^ (11 * a) * 2 ^ (- (3 : ℤ) * a) *
        ((1 + nndist_(p') (𝒬 p') (𝒬 p))^(-(2 * a^2 + a^3 : ℝ)⁻¹)) /
      ...
      push_cast
      ring_nf
    _ = 2 ^ ((2 * CDN + 6 + CDN/4) * a^3 + 1) * 2 ^ (11 * a) *
        ((1 + nndist_(p') (𝒬 p') (𝒬 p))^(-(2 * a^2 + a^3 : ℝ)⁻¹)) /
...
-- Bound 6.2.29 using 6.2.32 and `4 ≤ a`.
lemma bound_6_2_29' (ha : 4 ≤ a) (p p' : 𝔓 X) (x2 : E p) : 2 ^ ((2 * CDN + 6 + CDN/4) * a^3 + 8 * a + 1) *
  ...
  have h4 : 2 ^ ((2 * CDN + 6 + CDN/4) * a ^ 3 + 1) * 2 ^ (11 * a) ≤ C6_1_5 a := C6_1_5_bound ha
  -- Inequality 6.2.32
...
lemma correlation_le_of_nonempty_inter (ha : 4 ≤ a) {p p' : 𝔓 X} (hle : 𝔰 p' ≤ 𝔰 p) {g : X → ℂ}
  ...
  have hI12 : ∀ (x1 : E p') (x2 : E p), I12 x1 x2 ≤
      (2^((2 * CDN + 6 + CDN/4) * a^3 + 8 * a + 1) * ((1 + edist_(p') (𝒬 p') (𝒬 p))^(-(2 * a^2 + a^3 : ℝ)⁻¹))) /
      (volume (ball (x2 : X) (D ^𝔰 p))) * ‖g ↑x1‖ₑ * ‖g ↑x2‖ₑ :=
    I12_le (by omega) p p' hle g hinter
  -- Bound 6.2.29 using 6.2.32 and `4 ≤ a`.
  have hle' : ∀ (x2 : E p), 2 ^ ((2 * CDN + 6 + CDN/4) * a^3 + 8 * a + 1) *
      ((1 + nndist_(p') (𝒬 p') (𝒬 p))^(-(2 * a^2 + a^3 : ℝ)⁻¹)) /
        (volume (ball (x2 : X) (D ^𝔰 p))) ≤ (C6_1_5 a) *
          ((1 + nndist_(p') (𝒬 p') (𝒬 p))^(-(2 * a^2 + a^3 : ℝ)⁻¹)) /
