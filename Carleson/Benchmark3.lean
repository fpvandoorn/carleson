...
With these definitions, we define `improperFourierIntegral` to be the map from functions `V → E` to
...
At present the only nontrivial lemma we prove is `improperFourierIntegral_continuous`, stating that the
...
open MeasureTheory Filter TopologicalSpace ENNReal
...
variable {𝕜 : Type*} [CommRing 𝕜] {V : Type*} [AddCommGroup V] [Module 𝕜 V] [MeasurableSpace V]
  ...
  {e : AddChar 𝕜 𝕊} {μ : Measure V} {L : V →ₗ[𝕜] W →ₗ[𝕜] 𝕜} {f : V → E} {r : ℂ}

section Defs

/-- The Fourier transform integral for `f : V → E`, with respect to a bilinear form `L : V × W → 𝕜`
and an additive character `e`. -/
def improperFourierIntegral (e : AddChar 𝕜 𝕊) (μ : Measure V) (L : V →ₗ[𝕜] W →ₗ[𝕜] 𝕜)
    (f : V → E) (w : W) : E :=
  limUnder atTop (fun K : Compacts V ↦ ∫ v in K, e (-L v w) • f v ∂μ)

theorem improperFourierIntegral_eq_fourierIntegral (hf : Integrable f μ) :
    improperFourierIntegral e μ L f = fourierIntegral e μ L f := by
  sorry

theorem tendsto_improperFourierIntegral {p : ℝ≥0∞} (hf : MemLp f p μ)
    (h1p : 1 ≤ p) (h2p : p ≠ ∞) :
    Tendsto ((fun K : Compacts V ↦ ∫ v in K, e (-L v w) • f v ∂μ)) atTop
      (improperFourierIntegral e μ L f) := by
  sorry

theorem improperFourierIntegral_const_smul :
    improperFourierIntegral e μ L (r • f) = r • improperFourierIntegral e μ L f := by
  sorry
  -- ext1 w
  -- simp only [Pi.smul_apply, improperFourierIntegral, smul_comm _ r, integral_smul]

/-- The uniform norm of the Fourier integral of `f` is bounded by the `L¹` norm of `f`. -/
theorem norm_improperFourierIntegral_le_integral_norm (e : AddChar 𝕜 𝕊) (μ : Measure V)
    (L : V →ₗ[𝕜] W →ₗ[𝕜] 𝕜) (f : V → E) (w : W) :
    ‖improperFourierIntegral e μ L f w‖ₑ ≤ ∫⁻ v : V, ‖f v‖ₑ ∂μ := by
  sorry
  -- refine (norm_integral_le_integral_norm _).trans (le_of_eq ?_)
  -- simp_rw [Circle.norm_smul]

/-- The Fourier integral converts right-translation into scalar multiplication by a phase factor. -/
theorem improperFourierIntegral_comp_add_right [MeasurableAdd V] [μ.IsAddRightInvariant] (v₀ : V) :
    improperFourierIntegral e μ L (f ∘ fun v ↦ v + v₀) =
      fun w ↦ e (L v₀ w) • improperFourierIntegral e μ L f w := by
  sorry
  -- ext1 w
  -- dsimp only [improperFourierIntegral, Function.comp_apply, Circle.smul_def]
...
theorem improperFourierIntegral_convergent_iff (he : Continuous e)
    (hL : Continuous fun p : V × W ↦ L p.1 p.2) {f : V → E} (w : W) :
    (∀ K : Set V, IsCompact K → IntegrableOn (fun v : V ↦ e (-L v w) • f v) K μ) ↔
...
theorem improperFourierIntegral_add (he : Continuous e) (hL : Continuous fun p : V × W ↦ L p.1 p.2)
    {f g : V → E} (hf : Integrable f μ) (hg : Integrable g μ) :
    improperFourierIntegral e μ L (f + g) = improperFourierIntegral e μ L f + improperFourierIntegral e μ L g := by
  ext1 w
  dsimp only [Pi.add_apply, improperFourierIntegral]
  simp_rw [smul_add]
  rw [integral_add]
  · exact (improperFourierIntegral_convergent_iff he hL w).2 hf
  · exact (improperFourierIntegral_convergent_iff he hL w).2 hg

/-- The Fourier integral of an `L^1` function is a continuous function. -/
theorem improperFourierIntegral_continuous [FirstCountableTopology W] (he : Continuous e)
    (hL : Continuous fun p : V × W ↦ L p.1 p.2) {f : V → E} (hf : Integrable f μ) :
    Continuous (improperFourierIntegral e μ L f) := by
  apply continuous_of_dominated
  · exact fun w ↦ ((improperFourierIntegral_convergent_iff he hL w).2 hf).1
...
theorem integral_bilin_improperFourierIntegral_eq_flip
    {f : V → E} {g : W → F} (M : E →L[ℂ] F →L[ℂ] G) (he : Continuous e)
    (hL : Continuous fun p : V × W ↦ L p.1 p.2) (hf : Integrable f μ) (hg : Integrable g ν) :
    ∫ ξ, M (improperFourierIntegral e μ L f ξ) (g ξ) ∂ν =
      ∫ x, M (f x) (improperFourierIntegral e ν L.flip g x) ∂μ := by
  ...
  _ = ∫ ξ, (∫ x, M.flip (g ξ) (e (-L x ξ) • f x) ∂μ) ∂ν := by
    ...
    exact (improperFourierIntegral_convergent_iff he hL _).2 hf
  ...
  _ = ∫ x, M (f x) (∫ ξ, e (-L.flip ξ x) • g ξ ∂ν) ∂μ := by
    ...
    apply (improperFourierIntegral_convergent_iff he _ _).2 hg
    exact hL.comp continuous_swap

/-- The Fourier transform satisfies `∫ 𝓕 f * g = ∫ f * 𝓕 g`, i.e., it is self-adjoint. -/
theorem integral_improperFourierIntegral_smul_eq_flip
    {f : V → ℂ} {g : W → F} (he : Continuous e)
    (hL : Continuous fun p : V × W ↦ L p.1 p.2) (hf : Integrable f μ) (hg : Integrable g ν) :
    ∫ ξ, (improperFourierIntegral e μ L f ξ) • (g ξ) ∂ν =
      ∫ x, (f x) • (improperFourierIntegral e ν L.flip g x) ∂μ :=
  integral_bilin_improperFourierIntegral_eq_flip (ContinuousLinearMap.lsmul ℂ ℂ) he hL hf hg
...
theorem improperFourierIntegral_continuousLinearMap_apply
    {f : V → (F →L[ℝ] E)} {a : F} {w : W} (he : Continuous e) (hf : Integrable f μ) :
    improperFourierIntegral e μ L.toLinearMap₂ f w a =
      improperFourierIntegral e μ L.toLinearMap₂ (fun x ↦ f x a) w := by
  rw [improperFourierIntegral, ContinuousLinearMap.integral_apply]
  · rfl
  · apply (improperFourierIntegral_convergent_iff he _ _).2 hf
    exact L.continuous₂

theorem improperFourierIntegral_continuousMultilinearMap_apply
    {f : V → (ContinuousMultilinearMap ℝ M E)} {m : (i : ι) → M i} {w : W} (he : Continuous e)
    (hf : Integrable f μ) :
    improperFourierIntegral e μ L.toLinearMap₂ f w m =
      improperFourierIntegral e μ L.toLinearMap₂ (fun x ↦ f x m) w := by
  rw [improperFourierIntegral, ContinuousMultilinearMap.integral_apply]
  · rfl
  · apply (improperFourierIntegral_convergent_iff he _ _).2 hf
    exact L.continuous₂
...
character `e`. -/
def improperFourierIntegral (e : AddChar 𝕜 𝕊) (μ : Measure 𝕜) (f : 𝕜 → E) (w : 𝕜) : E :=
  VectorFourier.improperFourierIntegral e μ (LinearMap.mul 𝕜 𝕜) f w

theorem improperFourierIntegral_def (e : AddChar 𝕜 𝕊) (μ : Measure 𝕜) (f : 𝕜 → E) (w : 𝕜) :
    improperFourierIntegral e μ f w = ∫ v : 𝕜, e (-(v * w)) • f v ∂μ :=
  rfl

theorem improperFourierIntegral_const_smul (e : AddChar 𝕜 𝕊) (μ : Measure 𝕜) (f : 𝕜 → E) (r : ℂ) :
    improperFourierIntegral e μ (r • f) = r • improperFourierIntegral e μ f :=
  VectorFourier.improperFourierIntegral_const_smul _ _ _ _ _

/-- The uniform norm of the Fourier transform of `f` is bounded by the `L¹` norm of `f`. -/
theorem norm_improperFourierIntegral_le_integral_norm (e : AddChar 𝕜 𝕊) (μ : Measure 𝕜)
    (f : 𝕜 → E) (w : 𝕜) : ‖improperFourierIntegral e μ f w‖ ≤ ∫ x : 𝕜, ‖f x‖ ∂μ :=
  VectorFourier.norm_improperFourierIntegral_le_integral_norm _ _ _ _ _

/-- The Fourier transform converts right-translation into scalar multiplication by a phase
factor. -/
theorem improperFourierIntegral_comp_add_right [MeasurableAdd 𝕜] (e : AddChar 𝕜 𝕊) (μ : Measure 𝕜)
    [μ.IsAddRightInvariant] (f : 𝕜 → E) (v₀ : 𝕜) :
    improperFourierIntegral e μ (f ∘ fun v ↦ v + v₀) = fun w ↦ e (v₀ * w) • improperFourierIntegral e μ f w :=
  VectorFourier.improperFourierIntegral_comp_add_right _ _ _ _ _
...
theorem vector_improperFourierIntegral_eq_integral_exp_smul {V : Type*} [AddCommGroup V] [Module ℝ V]
    [MeasurableSpace V] {W : Type*} [AddCommGroup W] [Module ℝ W] (L : V →ₗ[ℝ] W →ₗ[ℝ] ℝ)
    (μ : Measure V) (f : V → E) (w : W) :
    VectorFourier.improperFourierIntegral fourierChar μ L f w =
      ∫ v : V, Complex.exp (↑(-2 * π * L v w) * Complex.I) • f v ∂μ := by
  simp_rw [VectorFourier.improperFourierIntegral, Circle.smul_def, Real.fourierChar_apply, mul_neg,
    neg_mul]
...
space, see `Real.improperFourierIntegral_convergent_iff`. -/
@[simp]
theorem improperFourierIntegral_convergent_iff' {V W : Type*} [NormedAddCommGroup V] [NormedSpace ℝ V]
  ...
  VectorFourier.improperFourierIntegral_convergent_iff (E := E) (L := L.toLinearMap₂)
...
theorem improperFourierIntegral_continuousLinearMap_apply'
    ...
    VectorFourier.improperFourierIntegral 𝐞 μ L.toLinearMap₂ f w a =
      VectorFourier.improperFourierIntegral 𝐞 μ L.toLinearMap₂ (fun x ↦ f x a) w :=
  VectorFourier.improperFourierIntegral_continuousLinearMap_apply continuous_fourierChar hf

theorem improperFourierIntegral_continuousMultilinearMap_apply'
    {f : V → ContinuousMultilinearMap ℝ M E} {m : (i : ι) → M i} {w : W} (hf : Integrable f μ) :
    VectorFourier.improperFourierIntegral 𝐞 μ L.toLinearMap₂ f w m =
      VectorFourier.improperFourierIntegral 𝐞 μ L.toLinearMap₂ (fun x ↦ f x m) w :=
  VectorFourier.improperFourierIntegral_continuousMultilinearMap_apply continuous_fourierChar hf
...
@[simp] theorem improperFourierIntegral_convergent_iff {μ : Measure V} {f : V → E} (w : V) :
    Integrable (fun v : V ↦ 𝐞 (- ⟪v, w⟫) • f v) μ ↔ Integrable f μ :=
  improperFourierIntegral_convergent_iff' (innerSL ℝ) w
...
def improperFourierIntegral (f : V → E) (w : V) : E :=
  VectorFourier.improperFourierIntegral 𝐞 volume (innerₗ V) f w
...
Denoted as `𝓕⁻¹` within the `Real.FourierTransform` namespace. -/
def improperFourierIntegralInv (f : V → E) (w : V) : E :=
  VectorFourier.improperFourierIntegral 𝐞 volume (-innerₗ V) f w

@[inherit_doc] scoped[FourierTransform] notation "𝓕" => Real.improperFourierIntegral
@[inherit_doc] scoped[FourierTransform] notation "𝓕⁻" => Real.improperFourierIntegralInv

lemma improperFourierIntegral_eq (f : V → E) (w : V) :
    𝓕 f w = ∫ v, 𝐞 (-⟪v, w⟫) • f v := rfl

lemma improperFourierIntegral_eq' (f : V → E) (w : V) :
    𝓕 f w = ∫ v, Complex.exp ((↑(-2 * π * ⟪v, w⟫) * Complex.I)) • f v := by
  simp_rw [improperFourierIntegral_eq, Circle.smul_def, Real.fourierChar_apply, mul_neg, neg_mul]

lemma improperFourierIntegralInv_eq (f : V → E) (w : V) :
    𝓕⁻ f w = ∫ v, 𝐞 ⟪v, w⟫ • f v := by
  simp [improperFourierIntegralInv, VectorFourier.improperFourierIntegral]

lemma improperFourierIntegralInv_eq' (f : V → E) (w : V) :
    𝓕⁻ f w = ∫ v, Complex.exp ((↑(2 * π * ⟪v, w⟫) * Complex.I)) • f v := by
  simp_rw [improperFourierIntegralInv_eq, Circle.smul_def, Real.fourierChar_apply]

lemma improperFourierIntegral_comp_linearIsometry (A : W ≃ₗᵢ[ℝ] V) (f : V → E) (w : W) :
    𝓕 (f ∘ A) w = (𝓕 f) (A w) := by
  simp only [improperFourierIntegral_eq, ← A.inner_map_map, Function.comp_apply,
    ← MeasurePreserving.integral_comp A.measurePreserving A.toHomeomorph.measurableEmbedding]

lemma improperFourierIntegralInv_eq_improperFourierIntegral_neg (f : V → E) (w : V) :
    𝓕⁻ f w = 𝓕 f (-w) := by
  simp [improperFourierIntegral_eq, improperFourierIntegralInv_eq]

lemma improperFourierIntegralInv_eq_improperFourierIntegral_comp_neg (f : V → E) :
    𝓕⁻ f = 𝓕 (fun x ↦ f (-x)) := by
  ext y
  rw [improperFourierIntegralInv_eq_improperFourierIntegral_neg]
  change 𝓕 f (LinearIsometryEquiv.neg ℝ y) = 𝓕 (f ∘ LinearIsometryEquiv.neg ℝ) y
  exact (improperFourierIntegral_comp_linearIsometry _ _ _).symm

lemma improperFourierIntegralInv_comm (f : V → E) :
    𝓕 (𝓕⁻ f) = 𝓕⁻ (𝓕 f) := by
  conv_rhs => rw [improperFourierIntegralInv_eq_improperFourierIntegral_comp_neg]
  simp_rw [← improperFourierIntegralInv_eq_improperFourierIntegral_neg]

lemma improperFourierIntegralInv_comp_linearIsometry (A : W ≃ₗᵢ[ℝ] V) (f : V → E) (w : W) :
    𝓕⁻ (f ∘ A) w = (𝓕⁻ f) (A w) := by
  simp [improperFourierIntegralInv_eq_improperFourierIntegral_neg, improperFourierIntegral_comp_linearIsometry]

theorem improperFourierIntegral_real_eq (f : ℝ → E) (w : ℝ) :
    improperFourierIntegral f w = ∫ v : ℝ, 𝐞 (-(v * w)) • f v :=
  rfl

theorem improperFourierIntegral_real_eq_integral_exp_smul (f : ℝ → E) (w : ℝ) :
    𝓕 f w = ∫ v : ℝ, Complex.exp (↑(-2 * π * v * w) * Complex.I) • f v := by
  simp_rw [improperFourierIntegral_real_eq, Circle.smul_def, Real.fourierChar_apply, mul_neg, neg_mul,
    mul_assoc]
...
theorem improperFourierIntegral_continuousLinearMap_apply
  ...
  improperFourierIntegral_continuousLinearMap_apply' (L := innerSL ℝ) hf

theorem improperFourierIntegral_continuousMultilinearMap_apply {ι : Type*} [Fintype ι]
    {M : ι → Type*} [∀ i, NormedAddCommGroup (M i)] [∀ i, NormedSpace ℝ (M i)]
    {f : V → ContinuousMultilinearMap ℝ M E} {m : (i : ι) → M i} {v : V} (hf : Integrable f) :
    𝓕 f v m = 𝓕 (fun x ↦ f x m) v :=
  improperFourierIntegral_continuousMultilinearMap_apply' (L := innerSL ℝ) hf
