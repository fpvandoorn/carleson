...
lemma iHolENorm_le_of_support_subset [ProofData a q K σ₁ σ₂ F G] (ha : 0 < a) (ϕ : X → ℂ) (x₀ : X) {r R : ℝ}
    (hr0 : 0 < r) (hrR : r ≤ R) (hϕr : ϕ.support ⊆ ball x₀ r) :
    iHolENorm ϕ x₀ R ≤ (ENNReal.ofReal R / ENNReal.ofReal r) ^ τ * iHolENorm ϕ x₀ r := by
  simp only [iHolENorm, mem_ball, defaultτ, ne_eq, mul_add]
  apply add_le_add
  · have : ⨆ x, ⨆ (_ : dist x x₀ < R), ‖ϕ x‖ₑ = ⨆ x, ⨆ (_ : dist x x₀ < r), ‖ϕ x‖ₑ := by
      congr
      ext x
      rw [iSup_eq_of_forall_le_of_forall_lt_exists_gt]
      · intro hx'
        by_cases hx : dist x x₀ < r
        · exact le_iSup_of_le hx (le_refl _)
        · have hϕx : ϕ x = 0 := by by_contra h; exact hx (hϕr h)
          rw [hϕx, enorm_zero, iSup_zero]
      · intro _ hy
        simp only [lt_iSup_iff, exists_prop] at hy ⊢
        exact ⟨lt_of_lt_of_le hy.1  hrR, hy.2⟩
    rw [this]
    apply le_mul_of_one_le_left'
    rw [le_rpow_inv_iff (Nat.cast_pos.mpr ha), rpow_natCast, one_pow,
      ENNReal.le_div_iff_mul_le (Or.inl (by simp [hr0])) (Or.inl ofReal_ne_top), one_mul]
    exact ofReal_le_ofReal hrR
  · rw [← mul_assoc]
    gcongr
    · apply le_of_eq
      have : (ENNReal.ofReal R / ENNReal.ofReal r) ^ (a : ℝ)⁻¹ * ENNReal.ofReal r ^ (a : ℝ)⁻¹ =
        (ENNReal.ofReal R / ENNReal.ofReal r * ENNReal.ofReal r) ^ (a : ℝ)⁻¹ := by
        rw [mul_rpow_eq_ite, if_neg (by simp)]
      rw [this]
      congr
      rw [ENNReal.div_mul_cancel (by simp [hr0]) ofReal_ne_top]
    · simp only [iSup_eq_dif, bot_eq_zero', dite_eq_ite]
      split_ifs with hR hr
      · sorry
      · simp only [nonpos_iff_eq_zero, iSup_eq_zero, ite_eq_right_iff, ENNReal.div_eq_zero_iff,
        enorm_eq_zero, rpow_eq_top_iff, inv_neg'', inv_pos, Nat.cast_pos]
        intro y hy hyx
    ...
    · ext x
      ...
      split_ifs with hR hr
      · sorry
      · simp only [iSup_eq_zero, ite_eq_right_iff, ENNReal.div_eq_zero_iff, enorm_eq_zero,
        rpow_eq_top_iff, inv_neg'', inv_pos, Nat.cast_pos]
        intro y hy hyx
        sorry
      · exfalso
        sorry
