...
lemma integral_nnreal_eq_integral_Ici_ofReal {f : ℝ≥0 → ℝ≥0∞} : ∫⁻ x : ℝ≥0, f x = ∫⁻ x in Ici (0 : ℝ), f x.toNNReal := by
  change ∫⁻ (x : ℝ≥0), f x = ∫⁻ (x : ℝ) in Ici 0, (f ∘ Real.toNNReal) x
  rw [← lintegral_subtype_comap measurableSet_Ici]
  simp
  rfl

lemma integral_nnreal_eq_integral_Ioi_ofReal {f : ℝ≥0∞ → ℝ≥0∞} : ∫⁻ x : ℝ≥0, f x = ∫⁻ x in Ioi (0 : ℝ), f (.ofReal x) := by
  rw [integral_nnreal_eq_integral_Ici_ofReal]
  exact setLIntegral_congr Ioi_ae_eq_Ici.symm

-- TODO: are there better names?
lemma integral_nnreal_eq_lintegral_toNNReal_Ioi (f : ℝ≥0 → ℝ≥0∞) :
    ∫⁻ x : ℝ≥0, f x = ∫⁻ x in Ioi (0 : ℝ), f x.toNNReal := by
  rw [integral_nnreal_eq_integral_Ici_ofReal]
  exact setLIntegral_congr Ioi_ae_eq_Ici.symm

-- TODO: do we actually use this?
lemma integral_nnreal_toReal_eq_integral_Ioi (f : ℝ → ℝ≥0∞) :
    ∫⁻ x : ℝ≥0, f (x.toReal) = ∫⁻ x in Ioi (0 : ℝ), f x := by
  rw [integral_nnreal_eq_lintegral_toNNReal_Ioi]
  refine setLIntegral_congr_fun_ae measurableSet_Ioi ?_
  filter_upwards with x hx
  have : max x 0 = x := max_eq_left_of_lt hx
  simp [this]

lemma integral_nnreal_toReal_eq_integral_Ici (f : ℝ → ℝ≥0∞) :
    ∫⁻ x : ℝ≥0, f (x.toReal) = ∫⁻ x in Ici (0 : ℝ), f x := by
  rw [integral_nnreal_toReal_eq_integral_Ioi]
