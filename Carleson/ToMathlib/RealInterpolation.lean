...
lemma estimate_eLpNorm_truncCompl {p q : ℝ≥0∞} [MeasurableSpace E₁] [BorelSpace E₁]
    (hp : p ≠ ⊤) (hpq : q ∈ Ioo 0 p) (hf : AEMeasurable f μ) {a : ℝ≥0∞} (ha : 0 < a) :
...
lemma lintegral_rpow_abs {j : Bool} {tc : ToneCouple} {γ : ℝ} {t : ℝ≥0∞}
    (hγ : if xor j tc.mon then γ > -1 else γ < -1 ) (ht : 0 < t) (ht' : t ≠ ⊤) :
...
lemma restrict_to_support {a : ℝ≥0∞} {p : ℝ} (hp : 0 < p) [NormedAddCommGroup E₁] (f : α → E₁) :
...
lemma restrict_to_support_truncCompl {t : ℝ≥0∞} {p : ℝ} [NormedAddCommGroup E₁] (hp : 0 < p)
    (f : α → E₁) :
...
lemma restrict_to_support_trnc {t : ℝ≥0∞} {p : ℝ} {j : Bool} [NormedAddCommGroup E₁] (hp : 0 < p)
    (f : α → E₁) :
