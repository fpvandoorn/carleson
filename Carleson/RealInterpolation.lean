...
lemma weaktype_estimate_trunc_compl_top {C₀ : ℝ≥0} (hC₀ : C₀ > 0) {p p₀ q₀ : ℝ≥0∞}
  ...
  · have p_pos : p > 0 := lt_trans hp₀ hp₀p
    have snorm_p_pos : eLpNorm f p μ ≠ 0 :=
      fun snorm_0 ↦ Ne.symm (ne_of_lt snorm_pos) (eLpNormEssSup_eq_zero_iff.mpr
        ((eLpNorm_eq_zero_iff hf.1 p_pos.ne.symm).mp snorm_0))
    have term_pos : (ENNReal.ofNNReal C₀) ^ p₀.toReal * eLpNorm f p μ ^ p.toReal > 0
        := by
      apply ENNReal.mul_pos <;> exact (rpow_pos_of_nonneg (by positivity) (by positivity)).ne.symm
    have term_ne_top : (ENNReal.ofNNReal C₀) ^ p₀.toReal * eLpNorm f p μ ^ p.toReal ≠ ⊤
        := mul_ne_top (rpow_ne_top' (ENNReal.coe_ne_zero.mpr hC₀.ne.symm) coe_ne_top)
    ...
    · nth_rw 2 [div_eq_mul_inv]
      ...
      · repeat rw [ENNReal.mul_rpow_of_ne_zero _ _ (q.toReal - q₁.toReal)]
        · rw [← ENNReal.rpow_neg, ← ENNReal.rpow_neg]
          ...
          · exact ne_zero_of_Ioo hF
          · exact ne_top_of_Ioo hF
          · exact (ENNReal.coe_pos.mpr hC₁).ne.symm
          · exact coe_ne_top
        · exact ENNReal.inv_ne_zero.mpr (rpow_ne_top' ((ENNReal.coe_pos.mpr hC₁).ne.symm) coe_ne_top)
        · exact ENNReal.inv_ne_zero.mpr (d_ne_top_aux₂ hF)
        · exact d_ne_zero_aux₁ hC₀
        · exact d_ne_zero_aux₀ hF
        · exact d_ne_zero_aux₂ hC₀ hF
