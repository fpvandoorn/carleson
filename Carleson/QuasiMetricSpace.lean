...
instance : Fact ((1 : ℝ) ≤ 1) := .mk le_rfl

/-- For now we are cheating: we are defining PseudoQuasiMetricSpaces to be pseudo metric spaces. -/
class PseudoQuasiMetricSpace (α : Type*) (A : outParam ℝ) [fact : Fact (1 ≤ A)] extends
...
class QuasiMetricSpace (α : Type*) (A : outParam ℝ) [fact : Fact (1 ≤ A)] extends
...
lemma dist_quasi_triangle (x y z : X) : dist x z ≤ A * (dist x y + dist y z) :=
  calc dist x z ≤ dist x y + dist y z := dist_triangle x y z
      ...
      _ ≤ A * (dist x y + dist y z) := by have : 1 ≤ A := Fact.out; gcongr
