...
\begin{itemize}
  \item Prerequisites: Fourier series, Hardy--Littlewood maximal function, doubling measures (\texttt{IsDoubling}), \texttt{WeakType}, \texttt{wnorm}. %TODO: HL only if needed in more than one other section
  %\item Real interpolation %TODO: what is the exact formulation used?
\end{itemize}
%TODO: include Lean notations for the described concepts?
We introduce a few basic concepts.

For a function $f : [0, 2\pi) \to \complex$, we define its $N$-th partial Fourier sum as
...
\end{equation}
%TODO: describe different types of integrals in mathlib
The coefficients $\widehat{f}_n$ are well-defined e.g. if $f\in L^1(0,2\pi)$.

%TODO: check differences to Lean definition
...
%TODO: what should we specify as codomain?
...
The function $f$ is said to be in weak $L^p$ (\lean{MemWLp}) if $f$ is $\mu$-a.e. strongly measurable with respect to a fixed topology on $E$ (see e.g. \cite{vanDoornFormalizedHaarMeasure, GouezelChangeOfVariables} for motivation of the different notions of measurability in mathlib) and $\|f\|_{L^{p,\infty}} < \infty$. %TODO: split this sentence, add short definition of aestronglymeasurable, and add reference to the definition in mathlib.
An important consequence is that if $f$ is in weak $L^p$, then $|f|$ is finite almost everywhere (see \lean{MemWLp.ae_ne_top}).
%TODO: add maybe other small statements (probably without proof)
%TODO: think about order of different definitions

%TODO: add that we use measure theoy from mathlib, cite other papers, e.g. about product measures, change of variables formula
Given two measure spaces $(X,\mu)$ and $(Y,\nu)$, and two topological spaces $E$ and $F$ endowed with extended norms, we say that an operator $T : (X \to E) \to (Y \to F)$ is of strong type $(p,p')$ if there exists a constant $C$ such that for any $f\in L^p(X; E)$, we have that $T f$ is $\nu$-a.e. strongly measurable and
\begin{equation}
