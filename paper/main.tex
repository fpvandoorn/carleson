...
which started in Fall 2023, by F.v.D. and the harmonic analysis group in Bonn, consisting of L.B., Christoph Thiele and Rajula Srivastava.

At the start L.B., Asgar Jamneshan, Rajula Srivastava and Christoph Thiele has written a preprint of the proof, that turned into [cite]. This preprint consisted of a 30-page proof of Theorem [ref].
...
This meant that the proof omitted many arguments that are routine to experts, but would require quite some work to figure out for outsiders.

After a very brief attempt to formalize the paper directly, we realized we needed a detailed blueprint that could be used as a foundation for the formalization.
The goal of the blueprint was that a non-expert in harmonic analysis could read the statement and proof of any lemma in the blueprint,
...
The proof in th blueprint was divided into 179 lemmas.
For most of these, the statement was formalized in Lean by F.v.D.,
which ensured a consistent statement of these lemmas,
and would make it easier to find the relevant definitions for the contributors.
To aid with this last point, we also kept a dictionary to record for notation in the blueprint the corresponding Lean name or notation.
F.v.D. also formalized most of the definitions used in the blueprint.

During the public formalization period, we used three platforms for communication and coordination.
We used GitHub to have a centralized version of the code,
and we used pull requests to ensure that at least one other contributor reviewed and approved additions or changes to the code.
...
We hosted a webpage, which mostly made an html-version of the blueprint available,
using Patrick Massot's \texttt{leanblueprint} software [cite/link]. This software generates a dependency graph, which gives a nice and quick overview of the progress of the project. It also generates for each lemma in the blueprint convenient links to the corresponding Lean lemma(s).

We used a public channel of Lean's Zulip chat to discuss the formalization.
...
In that case, another contributor or a member of Bonn's harmonic analysis group could help clarify the proof further.
In a few cases, an error was found that required fixes to the statement of one or more lemma statements in the blueprint, as described in Subsection \ref{subsec:blueprint_mistakes}.

We divided the blueprint into two categories: general material that would be useful in many other formalization projects,
and specific material to the current formalization, that would likely see little reuse.
We put the former material in a separete \texttt{ToMathlib} subfolder,
...
We relaxed some of the contribution requirements for the material specific to this formalization a little below \texttt{Mathlib}'s standards.
This enabled some contributors that only had some Lean experience to contribute to this project.

% maybe mention somewhere:
