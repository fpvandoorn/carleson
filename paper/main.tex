...
\begin{equation}\label{eq-freq-comp-ball}
    % \fcc(\fp)\in % todo: JP: I think it is easier to read if this is removed, please see if you agree
...
The statement of the hypotheses ($f : \R \to \C$ is a continuous $2\pi$-periodic function) closely resembles natural language, but understanding the conclusion requires some familiarity with Lean and Mathlib. There, $\forall^m x$ means `for every real $x$ away from a set of measure zero', and
...
\lean{n}-th Fourier coefficient of $f$ on $[0, 2\pi]$, and \lean{fourier n (x : AddCircle (2 * π))} denotes $e^{inx}$.
\begin{lstlisting}[mathescape]
...
Something to note here is that we created type synonyms to be able to endow $\Theta X$ with different metric space structures, each coming from a ball in $X$, and we provided the custom notation \lean{dist_\{x, r\}} for the distance corresponding to the ball $B(x, r)$.
...
\lean{linearizedCarlesonOperator}\href{https://github.com/fpvandoorn/carleson/blob/f2f614db47f1a30e3fceaa9c2e1f99ff356dd577/Carleson/Defs.lean#L212}{\extlink}; these definitions correspond to those introduced in equations (\ref{def-main-op}) and (\ref{def-T-r}), respectively.
