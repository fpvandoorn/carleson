...
One of the oldest questions in Fourier analysis consists on understanding the nature of convergence of Fourier series for different classes of functions (see Definition \ref{def:fourier-series}).
There are two main kinds of convergence that one may consider. First, one can ask whether the partial Fourier sums $S_N$ converge to the function $f$ in a suitable function
...
In \cite{fefferman}, Fefferman gave an alternative proof of Carleson's theorem via an a priori bound for Carleson's operator. 
In \cite{lacey-thiele}, a duality between the approaches by Carleson and Fefferman was pointed out and a more symmetric and self-dual proof was presented.
