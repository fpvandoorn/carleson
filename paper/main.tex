...
\subsection{Verification the definitions and statements}
...
Another aspects is making the relevant definitions easy to audit: to this end, we have collected all definitions and set-up necessary for stating the main theorems into a single file. Auditing these 160 lines of Lean code (on top of \mathlib) ensures the theorems say what they claim --- two orders of magnitude smaller than the entire project.
% 137 lines of Lean code (without comments) in BasicDefinitions, 26 in Challenge
In addition, about a quarter of these lines are basic definitions that will be added to \mathlib soon --- which implies getting careful review (and reducing the need to trust code beyond \mathlib further).

The astute reader may argue that the \emph{proofs} of these results require many more prerequisites, complicating this analysis: how can we ensure the actually proven statements match their theorems? We offer two answers:
firstly, we can use Lean's kernel to double-check: for each main theorem, we create a corresponding Lean definition whose type is the theorem statement. Down the line, we check that e.g. \texttt{linearized_carleson} actually provides a term of that type.
Secondly, the \emph{comparator} tool \url{https://github.com/leanprover/comparator} can verify such a correspondence of a given proof to a provided theorem, in a matter that is robust against malicious code or abuse of Lean's metaprogramming facilities. It is the gold standard for verification of formalisations.
We have verified the classical Carleson theorem (Theorem~\ref{classical-carleson}) and its metric space (Theorem~\ref{metric-space-Carleson}) and linearized metric space generalizations (Theorem~\ref{linearized-metric-Carleson}) using comparator: this certifies that the statements in the Lean file \texttt{Challenge.lean} match what is actually proven.
