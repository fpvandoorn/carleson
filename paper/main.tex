...
For a function $f$ on $X$, we define its weak $L^p$ norm to be
\begin{equation}
  \|f\|_{L^{p,\infty}} \coloneq \sup_{t>0} t \cdot \mu (\{x\in X : t < |f(x)| \}) ^ \frac{1}{p}
\end{equation}
if $0<p<\infty$, and $\|f\|_{L^{\infty,\infty}} \coloneq \|f\|_{L^{\infty}}$.
%TODO: add def of MemWLp, MemWLP.ae_ne_top (probably without proof), maybe other small statements


%TODO: include that T f has to be (ae)measurable
%TODO: add that we use measure theoy from mathlib, cite other papers, e.g. about product measures, change of variables formula
%TODO: formally specify domain of T?
An operator $T$ has weak type $(p, p')$ if there exists a constant $C$ such that for any $f\in L^p$,
\begin{equation}
  \|T f\|_{L^{p',\infty}} \le \|f\|_{L^p}.
\end{equation}
