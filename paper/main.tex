...
However, one also needs conversions in the other directions: sometimes one would need to convert real numbers to nonnegative real numbers. The conversions often play together with other operations, in that one for instance would need to know if, and often under which conditions, the product of the conversions is the conversion of the product. Indeed,
\texttt{
...
Yet there is no definition for $a^p$ if $p$ is in $\ennreal$, and therefore conversions to from
...
Fortunately, the tactic \texttt{ring} does work for these types.
The \texttt{norm\_num} does work for also for numbers in $\ennreal$, but not as well as for real numbers,
as even with the necessary side conditions it would not simplify a term of the form $a / a$.

As a final example around considerations on which number types to use,
consider the the supremum of a set.
...
During the project, we gradually built up intuition on where to put constants and where
