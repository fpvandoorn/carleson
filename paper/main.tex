...
% We define an ordering on tiles by $\fp\le \fp'$ iff
% $\scI(\fp)\le \scI(\fp')$ and $\Omega(\fp)\supset \Omega(\fp')$.
% For the next step, certain tiles are marked if
...
Four of these subclasses are \emph{antichains} with respect to a given ordering on the tiles.
