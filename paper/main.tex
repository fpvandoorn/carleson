...
The structures \lean{BoundedCompactSupport} and \lean{BoundedFiniteSupport} consist of functions that are essentially bounded measurable functions with compact support and with support of finite measure, respectively. A function $f$ is essentially bounded if there exists a real number $C$ such that $\|f(x)\|\le C$ almost everywhere.
...
(its support is a subset of the union of a compact set and a null set).

In Carleson, we originally started with functions that are bounded everywhere and have compact support,
but we switched (in pull requests 311 and 342) to functions that are only essentially bounded,
...
Another consideration into this design decision is using the functions in the class.
