...
Immler and Tan's formalization of the Poincaré-Bendixon theorem CITE (in Isabelle), % xxx noteworthy enough?
the flyspeck project \cite{FlyspeckComplete} formally verifying Hales' proof of the Kepler conjecture (verified using an interplay of results in the Isabelle/HOL and HOL/Light proof assistants), and the formalization of the Prime number theorem in HOL Light (by Harrison), Isabelle (by Avigad et al), Metamath (by Carneiro) and Lean (Kontorovich et al) \cite{}. % CITE ALL (or none)
% skipped four-colour theorem: large, but single person and more combinatorics than analysis or topology

The flyspeck project was also a large collaborative project, eventually involving 22 authors. This motivated the use of a mathematical blueprint, a detailed natural language proof carefully written to be accessible for non-experts, as shared basis for the formalisation. In the flyspeck project, this proof was written as a book prior to the formalization \cite{TODO}.
More recent formalization projects (including the Liquid Tensor Experiment \cites{LTEDefinitions,LTEFinal} and the Sphere Eversion project \cite{SphereEversionCPP) have used special software (blueprints, see Section~\ref{sec:org}) to maintain such a natural language proof side by side with the formal proof.

Other mathematical formalization projects involving large collaborations include the formal proof of the odd order theorem in the Rocq prover \cite{OddOrderTheorem}, the Liquid Tensor Experiment in Lean  (involving 18 authors), the PNT+ project\footnote{\url{https://alexkontorovich.github.io/PrimeNumberTheoremAnd/}} (with 59 contributors so far) and the Equational Theories project led by Terence Tao \cite{tao2025equationaltheories} (34 authors).
The last project is notable for involving a variety of different tools, all being used to generate eventual proofs in Lean.

Prior to this project, relatively little harmonic analysis had been formalized in any proof assistant: for example, the Isabelle and HOL Light libraries have an implementation of Fourier series (but no more advanced tools such as the Hardy-Littlewood maximal function or the real interpolation theorem); the Mizar Mathematical library has a formalization of $L^p$ spaces, but no definition of Fourier series (only discrete Fourier transforms).
...
% todo: maybe some deduplication is needed w.r.t. other sections
Before the formalization started, the blueprint~\cite{CarlesonBlueprint} was written
by the harmonic analysis group in Bonn, forming the mathematical basis of the formalization.
When the formalization project was announced, the blueprint was not finished in its entirety.
Some parts of Section~7 of the blueprint,
...
Most of the blueprint was finalized,
and contained more than enough material to start the formalization.

The main objective of the blueprint was to provide more detailed proofs for the lemmas and propositions
needed for the Carleson theorem.
...
One part of the formalization that we were worried about is the so-called ``invisible mathematics''
that one has to formalize.
% there doesn't seem to be anything good to cite here, except for slides by Andrej Bauer...
...
This approach also had downsides, as discussed later in this section.
