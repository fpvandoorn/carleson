...
but here we give an idea of the work it requires.

First, cubes are categorized by what fraction of it is covered by the set $G$.
...
This categorization lifts via the function $\scI$ to the tiles.
Second, we make a subcategorization of tiles $\fp$
by the fraction of the points $x$ in $\scI(\fp)$ for which $d_{\fp}(\tQ(x),\fcc(\fp))$ is small.
As a further subcategorization, some tiles are marked,
and we categorize tiles by how many marked tiles are ``near'' a given tile.
We should note that these characterizations also take into account the ordering on cubes and tiles.
The ordering on tiles is given by $\fp\le \fp'$ iff
$\scI(\fp)\le \scI(\fp')$ and $\Omega(\fp)\supset \Omega(\fp')$.

In each class in these categorizations, we mark certain tiles as \emph{tree tops}.
Then we eliminate 5 separate subclasses of tiles from the class.
Four of these subclasses are \emph{antichains}
with respect to the aforementioned ordering on the tiles.
