...
Of these three, the linearized version \Cref{linearised-metric-Carleson} implies the others. It is proved by first reducing it to a finitary version, which is then reduced to a discrete version.

Given the variables and assumptions of \Cref{linearised-metric-Carleson}, define
\[
D := 2^{100a^2}
\]
and let
...
we have formalised a more general argument that is also conceptually clearer: for instance, it demonstrates that the subtractive structure on the co-domain is not used in a meaningful way. We ended up with the following version.
\begin{theorem}
    Let $(X,\mu)$ and $(Y,\nu)$ be measure spaces, let $E$ be a topological space with the structure of a normed additive monoid endowed with a compatible extended seminorm (i.e. positive definite, fulfilling the triangle inequality) continuous extended norm, let $F$ be a topological space with a continuous extended norm function.
