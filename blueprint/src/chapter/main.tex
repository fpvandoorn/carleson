...
\begin{lemma}[Calderon-Zygmund Weak (1, 1)]
    ...
    \begin{equation}
        \label{eq-weak-1-1}
        \mu\left(\{x\in X: |T_r f(x)|>\alpha\}\right)\le \frac{2^{a^3+12a+4}}{\alpha} \int |f(y)|\, d\mu(y).
    \end{equation}
\end{lemma}
...
\label{second-cotlar-exception}
    |T_r(g\mathbf{1}_{B(x,\frac {R} 2)})(x')| > 2^{a^3 + 13a + 6} Mg(x)
\end{equation}
is less than or equal to  $\mu(B(x,\frac{R}{4}))/4$.
\end{lemma}
...
\begin{equation}
   \frac {2^{a^3+12a+4}}{2^{a^3+13a+6}Mg(x)}\int |g\mathbf{1}_{B(x,\frac {R}2)}|(y)\, d\mu(y)
\end{equation}
\begin{equation}
...
|T_Rg(x)| \le 2^{2}M(T_rg)(x)+ 2^{a^3+13a+7} Mg(x)
\, .
\end{equation}
\end{lemma}
...
by
\begin{equation}
    4 M(T_rg)(x) + 2^{a^3+13a+6} Mg(x) + 2^{a^3 + 4a + 1} Mg(x)\, .
\end{equation}
This proves the lemma.
\end{proof}
...
\begin{equation}
      T_{*}^r g(x)
     \le 2^{a^3+2a+2}Mg(x) + 2^{a^3+13a+7}Mg(x) + 2^{2}M(T_rg)(x)\, .
\end{equation}
Taking the $L^2$ norm and using \Cref{Hardy-Littlewood} with $a=4$  and $p_2=2$ and $p_1=1$ , we obtain
\begin{equation}
      \|T_{*}^r g\|_2
     \le 2^{a^3+13a+8} \|Mg\|_2 + 2^{2}\|M(T_rg)\|_2
\end{equation}
\begin{equation}
     \le 2^{a^3+17a+9} \|g\|_2 + 2^{4a+3}\|T_r g\|_2\, .
\end{equation}
Applying \eqref{two-sided-Hr-bound-assumption} gives
\begin{equation}
      \|T_{*}^r g\|_2\le 2^{a^3+17a+9}\|g\|_2 + 2^{a^3+4a+3}\|g\|_2\, .
\end{equation}
...
\begin{lemma}[Calderon Zygmund decomposition]
    ...
    Then there exists a bounded a.e. measurable function $g$ supported on a set of finite measure, a countable family of balls $B_j^*$ (where we allow $B_1^* = X$ in the special case that $\mu(X)<\infty$)
    ...
    \begin{equation}
        ...
        \sum_j \mu(B_j^*)\leq \frac{2^{6a}}{\alpha}\int |f(y)|\, d\mu(y)
    \end{equation}
    and
    \begin{equation}
    \label{eq-b-L1}
...
\begin{equation}
    \sum_j \mu(B_j^*) \le 2^{2a} \sum_j \mu(B_j) \le 2^{2a} \mu(E_\alpha) \le \frac{2^{6a}}{\alpha}\int |f(y)|\, d\mu(y),
\end{equation}
...
\begin{equation*}
    ...
    = \frac{2^{a^3+12a+4}}{\alpha} \int |f(y)|\, d\mu(y),
\end{equation*}
...
\begin{equation*}
    f=g+b=g+\sum_j b_j
\end{equation*}
such that the properties \eqref{eq-gb-dec}-\eqref{eq-b-L1} are satisfied (with $\alpha'$ replacing $\alpha$). We let
\begin{equation}
    \label{eq-Ij-cj}
    B_j=B(x_j, r_j).
\end{equation}
...
\begin{lemma}[Estimate F set]
    ...
    \begin{equation}
        ...
        \mu(\{x\in X\setminus\Omega: F(x)>\alpha/8\}) \le \frac{2^{-a}}{\alpha} \int |f(y)|\,d\mu(y)\,.
    \end{equation}
\end{lemma}
...
\begin{proof}
    ...
    Plugging this into \eqref{eq-F-est-1} and using \eqref{eq-bset-length-sum} and \eqref{weak-1-1-proof-cz-const}, we conclude that
    \begin{equation*}
        \mu(\{x\in X\setminus\Omega: F(x)> \alpha/8\}) \le \frac{2^{-a}}{\alpha} \int |f(y)|\,d\mu(y)\,.
    \end{equation*}
\end{proof}
...
\begin{lemma}[Estimate bad]
    ...
    We have
    \begin{equation*}
        \mu\left({\{x\in X: |T_r b(x)|>\alpha/2\}}\right) \le \frac{2^{5a} + 2^{-a}}{\alpha} \int |f(y)|\,d\mu(y) \,.
    \end{equation*}
\end{lemma}
...
\begin{proof}
    ...
    \begin{equation}
        \label{eq-omega-bd}
        \mu(\Omega) \le \sum_j \mu (B_j')
        \le 2^a \sum_j \mu(B_j) \le \frac{2^{5a}}{\alpha} \int |f(y)|\, d\mu(y)\,.
    \end{equation}
...
\begin{align*}
    \le& \frac{2^{2a^3+3a+2}c + 2^{5a} + 2^{-a}}{\alpha} \int |f(y)|\, d\mu(y) \\
    =& \frac{2^{a^3-9a-2} + 2^{5a} + 2^{-a}}{\alpha} \int |f(y)|\, d\mu(y) \\
    \le& \frac{2^{a^3+12a+4}}{\alpha} \int |f(y)|\, d\mu(y).
\end{align*}
