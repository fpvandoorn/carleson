...
\eqref{secondxx'c} by
%TODO: Justify last step.
\begin{equation}\label{secondxx'd}
\le  2^{a^3 + a} \cdot \frac{1}{1-2^{-\frac 1 a}} Mg(x) \le 2^{a^3 + 2a} Mg(x) \,.
\end{equation}
...
\begin{lemma}[Lebesgue differentiation]
    ...
    $$\frac{1}{\mu(B_n)}\int_{B_n} f(y)\, dy= f(x),$$
...
The following lemma corresponds to Lemma 3.2 in \cite{stein-book} with additional explicit statement and proof of the bounded intersection property, as well as countability of the disjoint family. The assumption on the boundedness of $O$ is not necessary but it simplifies the proof here because then we can use \Cref{Vitali-covering}, which is already in mathlib.
\begin{lemma}[Ball covering]
    \label{ball-covering}
    Given an open, bounded set $O\ne X$, there exists a countable family of balls $B_j = B(x_j, r_j)$ such that
    \begin{equation}
        \label{balls-disjoint}
        B_j \cap B_{j'} = \emptyset \quad \text{ for } j \ne j'
    \end{equation}
    and for $B_j^* := B(x_j, 6r_j)$,
    \begin{equation}
        \label{balls-covering}
        \bigcup_j B_j^* = O
    \end{equation}
    and for $B_j^{**} := B(x_j, 19r_j)$,
    \begin{equation}
        \label{enlarged-balls-intersect-complement}
        B_j^{**} \cap (X \setminus O) \ne \emptyset \quad \text{ for all } j
    \end{equation}
    and we have the bounded intersection property that each $x\in O$ is contained in at most $2^{7a}$ of the $B_j^*$.
\end{lemma}
\begin{proof}
Define for $x\in O$,
\begin{equation}
...
\end{equation}
Since $O$ is open, bounded, and $O\ne X$, we have
\begin{equation}
    0 < \delta(x) \le R
\end{equation}
for some constant $R\in\R$.
Applying \Cref{Vitali-covering} to $\{B(x,\frac{\delta(x)}{18}) : x \in O\}$, we obtain a family of balls $B_j = B(x_j, \frac{\delta(x_j)}{18}), j \in J$ such that \eqref{balls-disjoint}, \eqref{balls-covering} and \eqref{enlarged-balls-intersect-complement} all hold.
It remains to show that the family is countable and has the bounded intersection property.

For countability, we show that we can lower bound the volume of $B_j$ only in terms of its radius $r_j:=\frac{\delta(x_j)}{18}$ (and thus for all $q\in\Q_+$, there are only finitely many $j\in J$ with $r_j \ge q$): Since $O$ is bounded, there exists $\tilde x\in O$, $\tilde R>0$ such that $O\subset B(\tilde x, \tilde R)$. Now, for all $j\in J$,
\begin{equation*}
    \mu(B(\tilde x, \tilde R)) \le \mu(B(x_j, 2 \tilde R)) = \mu(B(x_j, \frac{2 \tilde R}{r_j} r_j))
    \le 2^{a\log_2{\lceil \frac{2 \tilde R}{r_j}\rceil}} \mu(B_j),
\end{equation*}
and thus
\begin{equation}
    \mu(B_j) \ge 2^{-a\log_2{\lceil \frac{2 \tilde R}{r_j}\rceil}} \mu(B(\tilde x, \tilde R)).
\end{equation}
%TODO: Provide more details for the previous step?
We now turn to the bounded intersection property. First observe that for $j,j'\in J$,  if $B_j^* \cap B_{j'}^* \ne \emptyset$, then
\begin{equation*}
    \delta(x) \le \rho(x,y) + \delta(y) \le (\frac{6\delta(x)}{19} + \frac{6\delta(y)}{19}) + \delta(y) \le \frac{\delta(x)}{3} + \frac{4\delta(a)}{3},
\end{equation*}
so
\begin{equation}
...
\end{equation}
Now assume that for some $j_1,\dots,j_N$,
\begin{equation}
    z\in \bigcap_{k=1}^N B_{j_k}^*.
\end{equation}
Then for all $1\le k \le N$, $B(z,\frac{\delta(z)}{18}) \subset B(x_{j_k}, 7r_j)$. Using \eqref{balls-disjoint} and \eqref{control-distance-growth}, we obtain
\begin{align}
    N \mu(B(z,\frac{\delta(z)}{18})) &\le \sum_{k=1}^N \mu(B(x_{j_k}, 7r_j)) \le 2^{3a} \sum_{k=1}^N \mu(B_{j_k}) \\
    &= 2^{3a} \mu(\bigcup_{k=1}^N B_{j_k}) \le 2^{3a} \mu(B(z,\frac{14\delta(z)}{18})) \le 2^{7a} \mu(B(z,\frac{\delta(z)}{18}))
\end{align}
and conclude $N\le 2^{7a}$.
% TODO: We used that $0<\mu(B(z,\frac{\delta(z)}{18}))<\infty$. Is that guaranteed in our setting?
\end{proof}
...
\begin{lemma}[Calderon Zygmund decomposition]
    ...
    Then there exists a measurable functions $g$, a countable family of closed balls $B_j$ such that each $x\in X$ is contained in at most $2^{7a}$ of the $B_j$, and a countable family of measurable functions $\{b_j\}_{j\in J}$ such that for almost every $x \in X$
    \begin{equation}
       \label{eq-gb-dec}
    ...
    and
     \begin{equation}
        \label{eq-bj-L1}
        \int_{B_j} |b_j(x)|\, dx \leq 2\alpha.
    \end{equation}
    We have
    \begin{equation}
        \label{eq-bset-length-sum}
        \sum_j \mu(B_j)\leq \frac{2}{\alpha}\int |f(y)|\, dy
    \end{equation}
    and
    \begin{equation}
    \label{eq-b-L1}
    \sum_{j}\int_{B_j} |b_j(y)|\, dy\leq 2 \int |f(y)|\, dy\,.
    \end{equation}
\end{lemma}
%TODO: Update proof and possibly statement
\begin{proof}
    ...
    %Assume first that $\mu(X) = \infty$.
    %TODO: Go on. Maybe the previous statement needs to be improved

    Assume now that $E_\alpha = X$.
    %Then $\mu(X)<\infty$.


    Applying \Cref{stopping-time} to $f$ and $\gamma\alpha$, we obtain a collection $B_j$ of intervals such that the conditions \eqref{eq-CZ-good-set}-\eqref{eq-CZ-meas-b-set} are satisfied. We set $A = \R \setminus \cup_j I_j$ and
\begin{equation}
    \label{eq-g-def}
    g(x):=\begin{cases}
     f(x), & x\in A,\\
...
\begin{equation}
    ...
    B_j{^*}=B(x_j, 5r_j).
\end{equation}
Then $B_j^*$ is an interval with the same center as $B_j$ but with
%TODO: correct this
\begin{equation}
    \label{eq-Ij*-dim}
...
$$ \mu\left(\{x\in X: |H_r g(x)|>{\alpha}/2\}\right)\leq
...
For $x\in X\setminus\Omega$, define
\begin{align*}
    \mathcal{J}_1(x)&:=\{j\,: B_j \cap [x-r, x+r]=\emptyset \},\\
