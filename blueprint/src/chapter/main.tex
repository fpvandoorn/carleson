...
\begin{proof}[Proof of \Cref{nontangential-operator-bound}]
    ...
    The first term \eqref{eq-sharp-trunc-term} is at most $2T_{\tQ}^\mfa f(x)$, using with $R_1 := 8D^{s_1}$, $R_2 := \frac{1}{4}D^{s_2-1}$ and
    \begin{equation}
    ...
    \end{equation}
    and making the second term larger by replacing $\rho(x,x')<R_1$ by $\rho(x,x')<R_2$.
