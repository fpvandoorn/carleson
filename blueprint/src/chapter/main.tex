...
\begin{lemma}[Calderon Zygmund decomposition]
    ...
    Then there exists a measurable function $g$, a countable family of balls $B_j^*$ such that each $x\in X$ is contained in at most $2^{6a}$ of the $B_j$, and a countable family of measurable functions $\{b_j\}_{j\in J}$ such that for all $x \in X$
    \begin{equation}
       \label{eq-gb-dec}
...
Let $B_1^*$ be a ball with $\supp f \subset B_1^*$ and $\mu(X) \le 2^{3a} \mu(B_1^*)$.
Define
\begin{equation*}
    g := \frac{\mathbb{1}_{\supp f}}{\mu(B_1^*)} \int |f(y)|\,d\mu(y)
\end{equation*}
and
\begin{equation*}
    b_1 := f - g.
