...
\proves{weak-1-1}
We set $\alpha':=c\alpha$ for a constant $c\ge 2^{-a^3-19a}$ which we will later choose explicitly.
If $\alpha'\le\frac{1}{\mu(X)}\int |f|\,d\mu$, then we directly have
\begin{equation*}
    \mu\left(\{x\in X: |H_r f(x)|>\alpha\}\right)\le \mu(X) \le \frac{1}{\alpha'} \int |f(y)|\, d\mu(y)
    \le \frac{2^{a^3 + 19a}}{\alpha} \int |f(y)|\, d\mu(y).
\end{equation*}
So assume now that $\alpha'>\frac{1}{\mu(X)}\int |f|\,d\mu$.
