...
\begin{lemma}[row bound]
    ...
    \end{equation}
    For each $1 \le j \le 2^n$ and each bounded $f$ with bounded support with $|f| \le \mathbf{1}_F$, we have
    \begin{equation}
        \label{eq-row-bound-2}
        \left\| \mathbf{1}_F T_{\mathfrak{R}_j}f \right\|_2 \le 2^{257a^3} 2^{-n/2} \dens_2(\bigcup_{\fu\in \fU}\fT(\fu))^{1/2} \|f\|_2\,.
    \end{equation}
\end{lemma}
...
\begin{proof}[Proof of \Cref{forest-operator}]
    ...
    $$
        \left\|\sum_{\fu \in \fU} \sum_{\fp \in \fT(\fu)} T_{\fp} f\right\|_2^2 = \left\|\sum_{j=1}^{2^n} \mathbf{1}_{E_j} T_{\mathfrak{R}_{j}} f\right\|_2^2 \le \sum_{j = 1}^{2^n} \|\mathbf{1}_{E_j} T_{\mathfrak{R}_{j}} f\|_2^2\,.
    $$
