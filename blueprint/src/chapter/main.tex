...
\lars{The material here is standard, we should probably call the `two assumption' by their common names} \lars{It would anyway be a good idea to explicitly say that we are doing Calderón-Zygmund theory here}
...
g(y) \kappa(x'-y)\, dy\right|\, .
\end{equation}
We now write by the triangle inequaqlity
...
\lars{Just do half open intervals everywhere, this avoids talking about interiors all the time}
...
\begin{lemma}[Calderón-Zygmund decomposition]
    \label{lem-czd-func}
...
\begin{proof}[Proof of Lemma \ref{lem-weak-1-1}]
Using Lemma \ref{lem-czd-func} for $f$ and $2^{-10}\alpha$, we obtain the deomposition
$$f=g+\sum_j b_j$$
