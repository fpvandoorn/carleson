...
\begin{theorem}[two-sided metric space Carleson]
    ...
    \uses{metric-space-Carleson}
        ...
        \label{two-sided-resweak}
            \left|\int_{G} T f \, \mathrm{d}\mu\right| \leq \frac{2^{Na^3}}{(q-1)^5} \mu(G)^{1-\frac{1}{q}} \mu(F)^{\frac{1}{q}}\, .
        \end{equation}
\end{theorem}
To prove this, we verify \eqref{nontanbound} in the following lemma (for a larger $a$ which we will have to account for in the final constant). It is proved in \Cref{subsec-cotlar}.
...
Let $0<r$ and  $x\in X$. Let $g$ be a bounded measurable function with bounded support on $X$. Let $Mg$ be the Hardy--Littlewood function
...
\begin{align}
 ...
 |g(y)| \frac{2^{a^3}}{\mu(B(x,2^j r))}\, dy \\
 &\le
...
\end{align}
Using a \Cref{geometric-series-estimate}, we estimate
\eqref{secondxx'c} by
...
Let $0<r<r_1$ and  $x\in X$. Let $g$ be a bounded measurable function with bounded support on $X$. Let $Mg$ be the Hardy--Littlewood function
...
\label{eqcotlar-1}
H_{r_1}(g)(x')=
\int_{r_1<\rho(x',y)} g(y) K(x',y) \, dy\, .
\end{equation}
On the domain $r_1<\rho(x',y)$, we have $\frac {r_1}2<\rho(x,y)$. Hence we may write
for \eqref{eqcotlar-1}
\begin{equation*}
H_{r_1}(g)(x')=\int_{r_1<\rho(x',y)} (g-g\mathbf{1}_{B(x,\frac {r_1} 2)})(y) K(x',y)\, dy
\end{equation*}
...
= \int_{ {r}<\rho(x',y)<r_1}
(g-g\mathbf{1}_{B(x,\frac {r_1} 2)})(y)K(x',y)\, dy\,  .
\end{equation}
...
Let $0<r<r_1$ and  $x\in X$. Let $g$ be a bounded measurable function with bounded support on $X$. Let $Mg$ be the Hardy--Littlewood maximal function
defined in Proposition
\ref{Hardy-Littlewood}.
Let $x\in X$.
...
\label{first-cotlar-exception}
    |H_{r}(g)(x')|> C M(H_{r}g)(x)
\end{equation}
is less than or equal to $\frac {\mu(B(x,\frac{r_1}{4}))}C$.
Moreover,  the measure $|F_2|$ of the set $F_2$ of all $x'\in
B(x,\frac {r_1} 4)$ such that
\begin{equation}
\label{second-cotlar-exception}
    |H_{r}(g\mathbf{1}_{B(x,\frac {r_1} 2)})| > D Mg(x)
\end{equation}
is less than $r_1/8$.
\end{lemma}
...
\begin{equation}
    \ge
    \frac 1{\mu(B(x,\frac{r_1}{4}))}\int_{F_1} C M(H_{r}g)(x)\, dx'
\end{equation}
Dividing by $M(H_{r}g)(x)$ gives
\begin{equation}
    1\ge \frac C{\mu(B(x,\frac{r_1}{4}))} |F_1|\, .
\end{equation}
This gives the desired bound for the measure of $F_1$.
% TODO: Go on here when weak 1-1 is done.
We turn to the set $F_2$. Similarly as above we may assume $Mg(x)>0$.
The set $F_2$ is then estimated with \Cref{weak-1-1}
by
\begin{equation}
   \frac {2^{19}}{D Mg(x)}\int |g\mathbf{1}_{[x-\frac {r_1}2, x+\frac {r_1}2]}|(y)\, dy
\end{equation}
\begin{equation}
   \le \frac {2^{19}}{D Mg(x)}r_1Mg(x)= \frac {r_1}8\,.
\end{equation}
...
Let $0<r<r_1$ and  $x\in X$. Let $g$ be a bounded measurable function with bounded support on $X$. Let $Mg$ and $M(H_r g)$ be the respective Hardy--Littlewood maximal functions
defined in \Cref{Hardy-Littlewood}.
Then for all $x\in X$
\begin{equation}\label{eq-cotlar-estimate}
|H_{r_1}g(x)| \le 2^{2}M(H_rg)(x)+ 2^{23} Mg(x)
\, .
\end{equation}
\end{lemma}
...
by
\begin{equation}
    C M(H_rg)(x) + D Mg(x) + 2^{a^3 + 2a + 3} Mg(x)\, .
\end{equation}
This proves the lemma.
\end{proof}
...
\uses{Hardy-Littlewood,Hilbert-strong-2-2,Cotlar-estimate}
    For every  $r>0$ and every bounded measurable function $g$ with bounded support we have
\begin{equation}\label{trzerobound}
    \|T_{r}g\|_2\le 2^{42}\|g\|_2,
\end{equation}
where
\begin{equation}\label{eq-simple--nontangential}
    T_{r} g(x):=\sup_{r<r_1}\sup_{\rho(x,x')<r_1}\frac 1{2\pi} |H_{r_1}(g)(x')| \, .
\end{equation}
\end{lemma}
\begin{proof}
...
\begin{equation}
     T_{r} g(x)
     \le 2^{a^3 + 2a + 2} Mg(x)+\sup_{r<r_1}
     \frac 1{2\pi} |H_{r_1}(g)(x)|\, .
\end{equation}
Using further \Cref{Cotlar-estimate},
we estimate
%TODO: Go on here when constants have been worked out
\begin{equation}
      T_{r} g(x)
     \le 2^{a^3 + 2a + 2} Mg(x)+2^{23}
     Mg(x)+2^{2}M(H_{r}g)(x)\, .
\end{equation}
...
\begin{equation}\label{eq-simpler--nontangential}
    T_{0} g(x):=\sup_{0<r_1}\sup_{\rho(x,x')<r_1}\frac 1{2\pi} \left|\int_{r_1<\rho(x',y)}
g(y) K(x',y)\, dy\right|\, .
\end{equation}
We now write by the triangle inequality
\begin{equation}\label{concretetstartriangle}
    T_* g(x)\le \sup_{0<r_1<r_2}\sup_{\rho(x,x')<r_1}\frac 1{2\pi} \left|\int_{r_1<\rho(x',y)}g(y) K(x',y)\, dy\right|
\end{equation}
    \begin{equation*}
+\sup_{0<r_1< r_2}\sup_{\rho(x,x')<r_1}\frac 1{2\pi} \left|\int_{r_2<\rho(x',y)} g(y) K(x',y)\, dy\right|\, .
\end{equation*}
...
\begin{equation}\label{concretetstartriangle2}
   \sup_{0<r_1}\sup_{\rho(x,x')<r_1}\frac 1{2\pi} \left|\int_{r_1<\rho(x',y)}g(y) K(x',y)\, dy\right|
\end{equation}
    \begin{equation*}
+\sup_{0<r_2}\sup_{\rho(x,x')<r_2}\frac 1{2\pi} \left|\int_{r_2<\rho(x',y)} g(y) K(x',y)\, dy\right|\, .
\end{equation*}
