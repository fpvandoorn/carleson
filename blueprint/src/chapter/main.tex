...
\begin{proof}
    ...
    \end{equation*}
    For $\lambda:=\frac{4}{a}$, we obtain
    \begin{equation*}
        2^{-\frac 1 a} \le 1 - (1-2^{-\frac 1 4}) \frac{4}{a}.
    \end{equation*}
    We conclude
    \begin{equation*}
        \sum_{n=0}^\infty 2^{-\frac{n}{a}} = \frac{1}{1-2^{-\frac 1 a}} \le \frac{1}{4(1-2^{-\frac 1 4})} a \le 2^a.
    \end{equation*}
\end{proof}
...
\label{Cotlar-sets}
\uses{weak-1-1}
Assume that \eqref{two-sided-Hr-bound-assumption} holds. Let $0<r<r_1$ and  $x\in X$. Let $g:X\to\C$ be a bounded measurable function supported on a set of finite measure. Let $Mg$ be the Hardy--Littlewood maximal function
defined in \Cref{Hardy-Littlewood}.
Let $x\in X$.
...
%fixme: this currently ignores the boundary of B(x',r_2). Are we sure that its measure is zero?
...
\end{lemma}
\begin{proof}
    %fixme: Do we know that X is nonempty?
    ...
    %fixme: We used that $0<\mu(B(x,Q)<\infty$. Is that guaranteed in our setting?
\end{proof}
...
% fixme: We used that $0<\mu(B(z,\frac{\delta(z)}{6}))<\infty$. Is that guaranteed in our setting?
\end{proof}
...
%fixme: How do we ensure that the sequences required in Lebesgue-differentiation exist for a.e. x?
...
%fixme: Could $\mu$ also be $\infty$ on every ball?
Define
\begin{equation*}
...
%fixme: verify assumption for $\mu(X)<\infty$
\begin{equation*}
    f=g+b=g+\sum_j b_j
\end{equation*}
...
%fixme: This is currently not the exact definition of H_r
\begin{equation*}
...
\end{equation*}
%fixme: Check constants. Maybe leave out the last explicit calculations.
...
\uses{real-Carleson,dirichlet-kernel}
