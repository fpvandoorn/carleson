...
\begin{theorem}[classical Carleson]
    ...
    Let $f$ be a $2\pi$-periodic complex-valued uniformly continuous function on $\mathbb{R}$. For all $0<\epsilon<1$, there exists a Borel set $E\subset [0,2\pi]$ with Lebesgue measure $|E|\le \epsilon$ and a positive integer $N_0$ such that for all $x\in [0,2\pi]\setminus E$ and all integers $N>N_0$, we have
...
%\uses{R-truncation}
    ...
    \end{equation}
    where $T_{\tQ}^\mfa$ is defined in
\eqref{def-lin-star-op}.
...
\begin{equation}
    \label{eq-TN-def}
    T_{\mathcal{N}}^\mfa f(x) := \sup_{-S \le s_1 < S} \sup_{x' \in I_{s_1}(x)} \sup_{\substack{s_1 < s_2 \le S\\ D^{s_2-1} \le R_Q(\mfa, x')}} \left| \sum_{s = s_1}^{s_2} \int K_s(x',y) f(y) \, \mathrm{d}\mu(y) \right|\,.
\end{equation}
