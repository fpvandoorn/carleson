...
\label{tile-structure}
 \uses{ball-metric-entropy,grid-existence,frequency-ball-cover, dyadic-frequency-cubes}
...
\uses{disjoint-frequency-cubes,frequency-cube-cover}
...
\end{equation}
%TODO: Should it be V(y,x) rather?
By the additional regularity, we can weaken the assumption \eqref{nontanbound} to an operator that is easier to work with in applications.
Namely, for $r > 0$, $x\in X$, and a bounded, measurable function $f$ on $X$ with bounded support, we define
\begin{equation}
\label{def-H_r}
H_r f(x):= \int_{r<\rho(x,y)} K(x,y) f(y) \, d\mu(y).
\end{equation}
\begin{theorem}[two-sided metric space Carleson]
    ...
    \uses{metric-space-Carleson}
        ...
        \begin{equation}\label{two-sided-Hr-bound}
            \|H_r g\|_{2} \leq 2^{a^3} \|g\|_2\,.
        \end{equation}
        Then for all Borel sets $F$ and $G$ in $X$ and
        all Borel functions $f:X\to \C$ with
        $|f|\le \mathbf{1}_F$, we have, with $T$ defined in \eqref{def-main-op},
        ...
        \label{two-sided-resweak}
            \left|\int_{G} T f \, \mathrm{d}\mu\right| \leq \frac{2^{1350a^3}}{(q-1)^5} \mu(G)^{1-\frac{1}{q}} \mu(F)^{\frac{1}{q}}\, .
        \end{equation}
\end{theorem}
To prove this, we verify \eqref{nontanbound} in the following lemma (for a larger $a$ which we have to account for in the final constant). It is proved in \Cref{subsec-cotlar}.
\begin{lemma}[nontangential-from-simple]\label{nontangential-from-simple}
    \uses{simple-nontangential}
    Assume that for every bounded measurable function $g : X \to \C$ supported on a set of finite measure and all $r>0$ we have
    \begin{equation*}
        \|H_r g\|_{2} \leq 2^{a^3} \|g\|_2\,.
    \end{equation*}
...
\begin{proof}[Proof of \Cref{two-sided-metric-space-Carleson}]
    ...
    \Cref{nontangential-from-simple} verifies \eqref{nontanbound} for $a':= 3^{\frac{1}{3}}a$ and all other assumptions still hold for this increased constant.
\end{proof}
...
\begin{lemma}[Weak 1 1]
    ...
    Let $f:X\to\C$ be a bounded measurable function supported on a set of finite measure and assume for some $r>0$ that
    \begin{equation}
        \label{eq-strong-2-2-assumption}
        \|H_rf\|_{2}\leq 2^{a^3} \|f\|_2.
    \end{equation}
    Then for all $\alpha>0$, we have
    \begin{equation}
        \label{eq-weak-1-1}
        \mu\left(\{x\in X: |H_r f(x)|>\alpha\}\right)\leq \frac{2^{a^3 + 19a}}{\alpha} \int |f(y)|\, d\mu(y).
    \end{equation}
\end{lemma}
Both \Cref{subsec-CZD,subsec-cotlar} use the Hardy--Littlewood maximal function defined in \Cref{Hardy-Littlewood} but other than that have no dependencies in the previous chapters.
...
Throughout this section we will often assume that the following holds:
For every bounded measurable function $f:X\to\C$ supported on a set of finite measure and all $r>0$ we have
\begin{equation}\label{two-sided-Hr-bound-assumption}
    \|H_f g\|_{2} \leq 2^{a^3} \|f\|_2\,.
\end{equation}
...
\begin{lemma}
    ...
    For all $a\ge 4$,
    \begin{equation*}
        \sum_{n=0}^\infty 2^{-\frac{n}{a}} \le 2^a.
    \end{equation*}
\end{lemma}
\begin{proof}
    \begin{equation*}
        \sum_{n=0}^\infty 2^{-\frac{n}{a}} = \frac{1}{1-2^{-\frac 1 a}}
    \end{equation*}
    %TODO: finish proof.
\end{proof}
...
Let $0<r$ and  $x\in X$. Let $g:X\to\C$ be a bounded measurable function supported on a set of finite measure. Let $Mg$ be the Hardy--Littlewood function
defined in Proposition \ref{Hardy-Littlewood}.
Then for all $x'$ with $\rho(x,x')<r$.
\begin{equation}\label{xx'difference}
\left| H_r g(x) - H_r g(x') \right|
\le 2^{a^3 + 2a + 2} Mg(x)\, .
\end{equation}
\end{lemma}
\begin{proof}
...
into the domains $r<\rho(x,y)\le 2r$
and $2r<\rho(x,y)$. The integral over the first domain we estimate
by $\eqref{firstxx'}$ below.
For the second domain, we
observe with $\rho(x,x')<r$ and the triangle inequality that $r<\rho(x'-y)$. We therefore combine on this domain with the
...
\begin{equation}
\label{firstxx'}
\int_{r<\rho(x,y)\le 2r}
|g(y)| |K(x,y)|\, d\mu(y)
\end{equation}
\begin{equation}\label{secondxx'}
  +  \left|\int_{2r<\rho(x,y)}
g(y) (K(x',y)-K(x,y))\, d\mu(y)
\right|
\end{equation}
\begin{equation}
\label{thirdxx'}
  +\left|\int_{r<\rho(x',y), r<\rho(x,y)<2r}
|g(y) K(x',y)|\, d\mu(y)
\right|
\end{equation}
...
\begin{align}
 \int_{r<\rho(x,y)<2r}
|g(y)| \frac{2^{a^3}}{V(x,y)}\, d\mu(y)\,
&\le
\frac{2^{a^3}}{\mu(B(x,r))} \int_{r<\rho(x,y)<2r}
|g(y)| \, d\mu(y)\, \\
\label{firstxx'b}
&\le
...
\begin{equation}\label{secondxx'b}
 \int_{2r<\rho(x,y)}
 |g(y)| \left(\frac{\rho(x,x')}{\rho(x,y)}\right)^{\frac{1}{a}}\frac{2^{a^3}}{V(x,y)} \, d\mu(y)
\end{equation}
We decompose and estimate
\eqref{secondxx'}
with the triangle inequality by
\begin{align}
 &\sum_{j=1}^\infty  \int_{2^jr<\rho(x,y)\le 2^{j+1}r}
 |g(y)| \left(\frac{\rho(x,x')}{\rho(x,y)}\right)^{\frac{1}{a}}\frac{2^{a^3}}{V(x,y)}\, d\mu(y)\, \\
 &\le
 \sum_{j=1}^\infty \left( 2^{-j} \right)^{\frac{1}{a}} \int_{2^jr<\rho(x,y)\le 2^{j+1}r}
 |g(y)| \frac{2^{a^3}}{\mu(B(x,2^j r))}\, d\mu(y) \\
 &\le
\sum_{j=1}^\infty 2^{-\frac{j}{a}} \frac{2^{a^3 + a}}{\mu(B(x,2^{j+1} r))} \int_{\rho(x,y)\le 2^{j+1}r}
|g(y)| \, d\mu(y) \\
\label{secondxx'c}
...
Let $0<r<r_1$ and  $x\in X$. Let $g:X\to\C$ be a bounded measurable function supported on a set of finite measure. Let $Mg$ be the Hardy--Littlewood function defined in \Cref{Hardy-Littlewood}.
Then for all $x'\in X$ with $\rho(x,x')<\frac {r_1}4$ we have
\begin{equation}\label{eq-cotlar-control}
\left|H_{r_1} g(x)
\right|\le
|H_{r}(g-g\mathbf{1}_{B(x,\frac {r_1} 2)})(x')| +
2^{a^3 + 2a + 3} Mg(x)\, .
\end{equation}
\end{lemma}

\begin{proof}
Let $x$ and $x'$ be given with $\rho(x,x')<\frac {r_1}4$.
...
\label{eqcotlar-1}
H_{r_1}(g)(x')=
\int_{r_1<\rho(x',y)} g(y) K(x',y) \, d\mu(y)\, .
\end{equation}
On the domain $r_1<\rho(x',y)$, we have $\frac {r_1}2<\rho(x,y)$. Hence we may write
for \eqref{eqcotlar-1}
\begin{equation*}
H_{r_1}(g)(x')=\int_{r_1<\rho(x',y)} (g-g\mathbf{1}_{B(x,\frac {r_1} 2)})(y) K(x',y)\, d\mu(y)
\end{equation*}
\begin{equation}\label{eqcotlar1}
=H_{r_1}(g-g\mathbf{1}_{B(x,\frac {r_1} 2)})(x')\, .
\end{equation}
...
We  have
\begin{equation*}
(H_{r}-H_{r_1})(g-g\mathbf{1}_{B(x,\frac {r_1} 2)})(x')
\end{equation*}
\begin{equation}\label{eqcotlar2}
= \int_{ {r}<\rho(x',y)<r_1}
(g-g\mathbf{1}_{B(x,\frac {r_1} 2)})(y)K(x',y)\, d\mu(y)\,  .
\end{equation}
Assume first $r\ge \frac {r_1} 4$.
Then we estimate \eqref{eqcotlar2} with
\eqref{eqkernel-size} by
\begin{equation*}
\int_{\frac {r_1}4<\rho(x',y)<r_1}
|g(y)K(x',y)|\, d\mu(y)
\end{equation*}
\begin{equation}\label{eqcotlar3}
\le \frac{2^{a^3}}{\mu(B(x',\frac{r_1}{4}))} \int_{\rho(x',y)<r_1}
|g(y)|\, d\mu(y) \le 2^{a^3 + 2a} Mg(x')\,  .
\end{equation}
Assume now $r\le \frac {r_1} 4$.
As $\rho(x',y)\le \frac{r_1}4$ implies
$\rho(x,y)\le \frac {r_1} 2$, we see that
...
\label{Cotlar-sets}
\uses{weak-1-1}
Assume that \eqref{eq-strong-2-2-assumption} holds. Let $0<r<r_1$ and  $x\in X$. Let $g:X\to\C$ be a bounded measurable function supported on a set of finite measure. Let $Mg$ be the Hardy--Littlewood maximal function
defined in \Cref{Hardy-Littlewood}.
Let $x\in X$.
Then the measure $|F_1|$ of the set $F_1$ of all $x'\in
B(x,\frac {r_1} 4)$ such that
\begin{equation}
\label{first-cotlar-exception}
    |H_{r}(g)(x')|> 4 M(H_{r}g)(x)
\end{equation}
is less than or equal to $\mu(B(x,\frac{r_1}{4}))/4$.
Moreover,  the measure $|F_2|$ of the set $F_2$ of all $x'\in
B(x,\frac {r_1} 4)$ such that
\begin{equation}
\label{second-cotlar-exception}
    |H_{r}(g\mathbf{1}_{B(x,\frac {r_1} 2)})(x')| > 2^{a^3 + 20a + 2} Mg(x)
\end{equation}
is less than or equal to  $\mu(B(x,\frac{r_1}{4}))/4$.
\end{lemma}
...
\begin{equation}
    M(H_{r}g)(x)\ge
    \frac 1{\mu(B(x,\frac{r_1}{4}))}\int_{\rho(x,x')<\frac {r_1}4}|H_{r}g(x')|\, dx'
\end{equation}
\begin{equation}
    \ge
    \frac 1{\mu(B(x,\frac{r_1}{4}))}\int_{F_1} 4 M(H_{r}g)(x)\, dx'
\end{equation}
Dividing by $M(H_{r}g)(x)$ gives
\begin{equation}
    1\ge \frac{4}{\mu(B(x,\frac{r_1}{4}))} |F_1|\, .
\end{equation}
...
\begin{equation}
   \frac {2^{a^3+19a}}{2^{a^3 + 20a + 2}Mg(x)}\int |g\mathbf{1}_{B(x,\frac {r_1}2)}|(y)\, d\mu(y)
\end{equation}
\begin{equation}
   \le \frac {1}{2^{a+2} Mg(x)}\mu(B(x,\frac {r_1}{2})) Mg(x) \le \frac{\mu(B(x,\frac {r_1}{4}))}{4} \,.
\end{equation}
...
Assume that \eqref{eq-strong-2-2-assumption} holds.
Let $0<r<r_1$ and  $x\in X$. Let $g$ be a bounded measurable function with bounded support on $X$ and assume . Let $Mg$ and $M(H_r g)$ be the respective Hardy--Littlewood maximal functions
defined in \Cref{Hardy-Littlewood}.
Then for all $x\in X$
\begin{equation}\label{eq-cotlar-estimate}
|H_{r_1}g(x)| \le 2^{2}M(H_rg)(x)+ 2^{a^3+20a+3} Mg(x)
\, .
\end{equation}
\end{lemma}
...
\begin{proof}
By \Cref{Cotlar-sets}, the measure of the set
of all $x'\in
B(x,\frac {r_1} 4)$
such that at least one of the conditions
\eqref{first-cotlar-exception} and
\eqref{second-cotlar-exception} is satisfied is at most $\mu(B(x,\frac{r_1}{4}))/2$ and hence not all of
 $x'\in
B(x,\frac {r_1} 4)$. Pick an
$x'\in
B(x,\frac {r_1} 4)$ such that both conditions are not satisfied.
...
by
\begin{equation}
    4 M(H_rg)(x) + 2^{a^3 + 20a + 2} Mg(x) + 2^{a^3 + 2a + 3} Mg(x)\, .
\end{equation}
This proves the lemma.
\end{proof}
...
\begin{equation}\label{trzerobound}
    \|T_{r}g\|_2\le 2^{a^3+24a+6}\|g\|_2,
\end{equation}
where
\begin{equation}\label{eq-simple--nontangential}
    T_{r} g(x):=\sup_{r<r_1}\sup_{\rho(x,x')<r_1} |H_{r_1}(g)(x')| \, .
\end{equation}
\end{lemma}
\begin{proof}
...
\begin{equation}
     T_{r} g(x)
     \le 2^{a^3 + 2a + 2} Mg(x)+\sup_{r<r_1} |H_{r_1}(g)(x)|\, .
\end{equation}
Using further \Cref{Cotlar-estimate}, we estimate
\begin{equation}
      T_{r} g(x)
     \le 2^{a^3+2a+2}Mg(x) + 2^{a^3+20a+3}Mg(x) + 2^{2}M(H_{r}g)(x)\, .
\end{equation}
Taking the $L^2$ norm and using \Cref{Hardy-Littlewood} with $a=4$  and $p_2=2$ and $p_1=1$ , we obtain
\begin{equation}
      \|T_{r} g\|_2
     \le 2^{a^3+20a+4} \|Mg\|_2 + 2^{2}\|M(H_{r}g)\|_2
\end{equation}
\begin{equation}
     \le 2^{a^3+24a+5} \|g\|_2 + 2^{4a+3}\|H_{r}(g)\|_2\, .
\end{equation}
Applying \eqref{eq-strong-2-2-assumption}, gives
\begin{equation}
...
\begin{equation}\label{eq-simpler--nontangential}
    T_{0} g(x):=\sup_{0<r_1}\sup_{\rho(x,x')<r_1} \left|\int_{r_1<\rho(x',y)}
g(y) K(x',y)\, d\mu(y)\right|\, .
\end{equation}
We now write by the triangle inequality
%TODO: this currently ignores out the boundary of B(x',r_2). Are we sure that its measure is zero?
\begin{equation}\label{concretetstartriangle}
    T_* g(x)\le \sup_{0<r_1<r_2}\sup_{\rho(x,x')<r_1} \left|\int_{r_1<\rho(x',y)}g(y) K(x',y)\, d\mu(y)\right|
\end{equation}
    \begin{equation*}
+\sup_{0<r_1< r_2}\sup_{\rho(x,x')<r_1} \left|\int_{r_2<\rho(x',y)} g(y) K(x',y)\, d\mu(y)\right|\, .
\end{equation*}
Noting that the first integral does not depend on $r_2$ and
estimating the second integral by the larger supremum over all
$\rho(x,x')<r_2$, at which time the integral does not depend on $r_1$, we estimate \eqref{concretetstartriangle} by
\begin{equation}\label{concretetstartriangle2}
   \sup_{0<r_1}\sup_{\rho(x,x')<r_1} \left|\int_{r_1<\rho(x',y)}g(y) K(x',y)\, d\mu(y)\right|
\end{equation}
    \begin{equation*}
+\sup_{0<r_2}\sup_{\rho(x,x')<r_2} \left|\int_{r_2<\rho(x',y)} g(y) K(x',y)\, d\mu(y)\right|\, .
\end{equation*}
...
\label{subsec-CZD}
The following lemma is Theorem 3.1(b) in \cite{stein-book}. The proof uses \Cref{Hardy-Littlewood} (and maybe it should become part of that?).
\begin{lemma} [Maximal theorem]
    \label{maximal-theorem}
    Let $f: X \to \C$ be bounded, measurable, with bounded support and let $\alpha > 0$. For $Mf$ as in \Cref{Hardy-Littlewood},
    \begin{equation}
...
\end{lemma}
\begin{proof}
    By definition, for each $x\in \{x\in X : Mf(x) > \alpha\}$, there exists a ball $B_x$ so that $x\in B_x$ and
    \begin{equation}
...
\begin{lemma}[Lebesgue differentiation]
    ...
    where $\{B_n\}_{n\geq 1}$ is a sequence of balls such that $x\in B_n$ for each $n\geq 1$ and $$\lim_{n\to \infty} \mu(B_n)=0.$$
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
    %TODO: Do we know that X is nonempty?
    ...
    %TODO: We used that $0<\mu(B(x,Q)<\infty$. Is that guaranteed in our setting?
\end{proof}
...
%Let $\bar B (x,r) := \{y\in X : \rho(x,y) \le r\}$ denote the closed ball with center $x$ and radius $r$.
...
The following lemma corresponds to Lemma 3.2 in \cite{stein-book} with additional explicit statement and proof of the bounded intersection property, as well as countability of the disjoint family.
...
\begin{equation}
    0 < \delta(x) < \infty
\end{equation}
By Zorn's Lemma, we select a maximal disjoint subfamily of $\{B(x,\frac{\delta(x)}{6}) : x \in O\}$.
We obtain a (by \Cref{disjoint-family-countable} countable) family of balls $B_j = B(x_j, \frac{\delta(x_j)}{6}), j \in J$ such that \eqref{balls-disjoint}. \eqref{enlarged-balls-intersect-complement} and $\bigcup_j B_j^* \subset O$ are also immediate. For the other inclusion, observe that for $x,y\in X$,  if $B(x,\frac{\delta(x)}{6}) \cap B(y,\frac{\delta(y)}{6}) \ne \emptyset$, then
\begin{equation*}
    \delta(x) \le \rho(x,y) + \delta(y) \le (\frac{\delta(x)}{6} + \frac{\delta(y)}{6}) + \delta(y) = \frac{\delta(x)}{6} + \frac{7\delta(a)}{6},
\end{equation*}
so
\begin{equation}
...
\begin{equation*}
    \rho(z,x_j)\le \frac{\delta(z)}{6} + \frac{\delta(x_j)}{6} \le \frac{3\delta(x_j)}{6} = 3r_j
\end{equation*}
and thus $z\in B_j^*$.
...
% TODO: We used that $0<\mu(B(z,\frac{\delta(z)}{6}))<\infty$. Is that guaranteed in our setting?
\end{proof}
...
\begin{lemma}[Calderon Zygmund decomposition]
    ...
    \uses{stopping-time}
    Let $f$ be a bounded, measurable function with bounded support and let $\alpha>\frac{1}{\mu(X)}\int |f|\,d\mu$.
...
\begin{proof}
Let $E_\alpha:=\{x\in X: Mf(x)>\alpha\}$, with $Mf$ as defined in \Cref{Hardy-Littlewood}.
Then $E_\alpha$ is open. Assume first that $E_\alpha \ne X$. We apply \Cref{ball-covering} with $O=E_\alpha$ to obtain the family $B_j, j\in J,$. Without loss of generality, we can assume $J=\N$. Define
\begin{equation}
    Q_j := B_j^* \setminus \left(\bigcup_{i<j} Q_i \cup \bigcup_{i>j} B_i \right).
\end{equation}
Then the $B_j\subset Q_j\subset B_j^*$, the $Q_j$ are pairwise disjoint and $\bigcup_j Q_j = E_\alpha$.
Define
\begin{equation}
    \label{eq-g-def}
    g(x):=\begin{cases}
...
and, for each $j$,
\begin{equation}
    b_j(x):= \mathbb{1}_{Q_j}(x) \left(f(x)-\frac{1}{\mu(Q_j)}\int_{Q_j} f(y)\, d\mu(y) \right).
\end{equation}
...
%TODO: How do we ensure that the sequences required in Lebesgue-differentiation exist for a.e. x?
...
Assume now that $E_\alpha = X$. Note that this can only happen if $\mu(X)<\infty$.
%TODO: Could $\mu$ also be $\infty$ on every ball?
Define
\begin{equation*}
...
\proves{weak-1-1}
Using \Cref{Calderon-Zygmund-decomposition} for $f$ and $\alpha':=c\alpha$, we obtain the decomposition
%TODO: verify assumption for $\mu(X)<\infty$
\begin{equation*}
    f=g+b=g+\sum_j b_j
\end{equation*}
such that the properties \eqref{eq-gb-dec}-\eqref{eq-b-L1} are satisfied (with $\alpha'$ replacing $\alpha$). We rename $B_j^*$ to $B_j$, and for each $j$, let
\begin{equation}
    \label{eq-Ij-cj}
    B_j=B(x_j, r_j).
\end{equation}
...
\begin{equation}
    ...
    \Omega:=\bigcup_j B_j'.
\end{equation}
By definition, for each $x\in X\setminus\Omega$ and $y\in B_j$,
\begin{equation}
    \label{eq-Om-cj}
...
\begin{equation*}
    \mu\left(\{x\in X: |H_r f(x)|>\alpha\}\right)
\end{equation*}
\begin{equation}
\label{eq-set-dec-1}
 \le \mu\left(\{x\in X: |H_r g(x)|>{\alpha}/2\}\right)+  \mu\left(\{x\in X: |H_r b(x)|>{\alpha}/2\}\right).
\end{equation}
We estimate using monotonicity of the integral
\begin{equation*}
     \mu\left(\{x\in X: |H_r g(x)|>{\alpha}/2\}\right)\leq \frac{4}{\alpha^2} \int |H_r g(y)|^2\, d\mu(y).
\end{equation*}
...
Next, we estimate
\begin{equation*}
    \mu\left(\{x\in X: |H_r b(x)|>\alpha/2\}\right)
\end{equation*}
\begin{equation}
    \label{eq-set-dec-2}
    \le \mu (\Omega) +  \mu\left(\{x\in X\setminus\Omega: |H_r b(x)|>{\alpha}/2\}\right)\,.
\end{equation}
...
\begin{equation*}
    \mu\left(\{x\in X\setminus\Omega: |H_r b(x)|>{\alpha}/2\}\right)\,.
\end{equation*}
Let $x\in X\setminus\Omega$. We decompose the index set $J$ into the following disjoint sets:
\begin{align*}
...
\begin{align}
    \label{eq-b-dec-1}
    |H_r b(x)|\le&\sum_{j\in \mathcal{J}_1(x)} |H_rb_j(x)| \\
    \label{eq-b-dec-2}
                +&\sum_{j\in \mathcal{J}_2(x)} |H_rb_j(x)| \\
    \label{eq-b-dec-3}
                +&\sum_{j\in \mathcal{J}_3(x)} |H_rb_j(x)|.
\end{align}
For all $j\in \mathcal{J}_3(x)$, $\supp b_j\subset B_j\subset B(x,r)$, and thus $H_rb_j(x)=0$, so $\eqref{eq-b-dec-3} = 0$.

Next, for $j\in \mathcal{J}_1(x)$, $\supp b_j\subset B_j\subset X \setminus B(x,r)$, and we have
\begin{equation*}
    H_rb_j(x)=\int_{X\setminus B(x,r)} K(x,y) b_j(y)\,d\mu(y)=\int_{B_j} K(x,y) b_j(y)\,d\mu(y)\,.
\end{equation*}
...
\end{equation*}
By \eqref{eq-Om-cj}, we can apply \eqref{eqkernel-y-smooth} to estimate
\begin{equation*}
...
\begin{align*}
    H_r b_j(x)&=\int_{B_j} K(x,y) (\mathbf{1}_{X\setminus B(x,r)}(y)b_j(y)-d_j)\, dy + \int_{B_j} d_j K(x,y) \, dy \\
    &= \int_{B_j} (K(x,y)-K(x,c_j)) (\mathbf{1}_{X\setminus B(x,r)}(y)b_j(y)-d_j)\, dy + \int_{B_j} d_j K(x,y) \, dy.
\end{align*}
...
\begin{equation}
    ...
    \int_{B_j} |(K(x,y)-K(x,c_j)) \left(|b_j(y)|+2^{2a+1} c\alpha\right)\, dy +2^{2a+1} c\alpha \int_{B_j}  |K(x,y)| \, dy.
\end{equation}
...
\end{equation}
Indeed, for each $j\in \mathcal{J}_2(x)$ and $y\in B_j$,
\begin{equation*}
    \rho(x,x_j) < r+r_j \le r + \frac{1}{2} \rho(x,x_j) \implies \rho(x,x_j) < 2r
\end{equation*}
and hence
\begin{equation*}
    \rho(x,y) \le \rho(x,x_j) + \rho(x_j,y) < 2r + r_j \le 2r + \frac{1}{2}\rho(x,x_j) \le 3r.
\end{equation*}
...
\begin{align}
    ...
    &\le 2^{a^3+3a} \int_{B(x,3r) \setminus B(x,\frac{r}{3})} \frac{1}{\mu(B(x,\frac{r}{3}))} \,d\mu(y) \\
...
\begin{equation*}
    |H_rb(x)|\leq 3F(x)+2^{a^3+12a+1}c\alpha.
\end{equation*}
With $c:=2^{-a^3-12a-4}$, the equals
\begin{equation*}
    3F(x)+\alpha/8.
\end{equation*}
...
\begin{equation}
    \label{eq-set-dec-3}
    \mu({\{x\in X\setminus\Omega: |H_r b(x)|>\alpha/2\}}) \le \mu(\{x\in X\setminus\Omega: F(x)> \alpha/8\}).
\end{equation}
...
\begin{equation}
    \label{eq-Hr-b-X-minus-Omega}
    \mu({\{x\in X\setminus\Omega: |H_r b(x)|>\alpha/2\}}) \le  \frac{2^{a^3+9a+4}}{\alpha} \int |f(y)|\,d\mu(y)\,.
\end{equation}
Combining estimates \eqref{eq-set-dec-1}, \eqref{eq-Hr-g}, \eqref{eq-set-dec-2}, \eqref{eq-omega-bd}, \eqref{eq-Hr-b-X-minus-Omega} yields
\begin{equation*}
    \mu\left(\{x\in X: |H_r f(x)|>\alpha\}\right) \le \frac{2^{2a^3+3a+2}c + \frac{2^{5a}}{c} + 2^{a^3+9a+4}}{\alpha} \int |f(y)|\, d\mu(y).
\end{equation*}
Using the definition of $c$, we bound this by
\begin{equation*}
    \frac{2^{a^3-9a-2} + 2^{a^3+17a+4} + 2^{a^3+9a+4}}{\alpha} \int |f(y)|\, d\mu(y) \le \frac{2^{a^3 + 19a}}{\alpha} \int |f(y)|\, d\mu(y).
\end{equation*}
%TODO: Check constants. Maybe leave out the last explicit calculations.
...
This latter proof refers to the two-sided variant of the Carleson \Cref{two-sided-metric-space-Carleson}. Two assumptions in \Cref{metric-space-Carleson} require more work. The boundedness of the operator $H_r$ defined in \eqref{def-H_r} is established in \ref{Hilbert-strong-2-2}. This lemma is proved in \Cref{10hilbert}.
...
\leanok
\uses{two-sided-metric-space-Carleson,real-line-metric,real-line-measure,real-line-doubling,oscillation-control,frequency-monotone,frequency-ball-doubling,frequency-ball-growth,integer-ball-cover,real-van-der-Corput,Hilbert-strong-2-2}
...
\Cref{Hilbert-strong-2-2} verifies \eqref{eq-strong-2-2-assumption}.
