...
\begin{equation}
\label{def-H_r}
H_r f(x):= \int_{r\le\rho(x,y)} K(x,y) f(y) \, d\mu(y) = \int_{X\setminus B(x,r)} K(x,y) f(y) \, d\mu(y).
\end{equation}
...
\begin{lemma}[Weak 1 1]
    ...
    \begin{equation}
        ...
        \|H_rf\|_{2}\le 2^{a^3} \|f\|_2.
    \end{equation}
    Then for all $\alpha>0$, we have
    \begin{equation}
        \label{eq-weak-1-1}
        \mu\left(\{x\in X: |H_r f(x)|>\alpha\}\right)\le \frac{2^{a^3 + 19a}}{\alpha} \int |f(y)|\, d\mu(y).
    \end{equation}
\end{lemma}
...
\begin{equation}
\label{thirdxx'}
  +\left|\int_{r\le\rho(x',y), r\le\rho(x,y)<2r}
|K(x',y) g(y)|\, d\mu(y)
\right|
\end{equation}
...
Let $0<r\le R$ and  $x\in X$. Let $g:X\to\C$ be a bounded measurable function supported on a set of finite measure. Let $Mg$ be the Hardy--Littlewood function defined in \Cref{Hardy-Littlewood}.
Then for all $x'\in X$ with $\rho(x,x')\le\frac {R}{4}$ we have
\begin{equation}\label{eq-cotlar-control}
\left|H_{R} g(x)
\right|\le
|H_{r}(g-g\mathbf{1}_{B(x,\frac {R} 2)})(x')| +
2^{a^3 + 4a + 1} Mg(x)\, .
\end{equation}
\end{lemma}
...
\begin{equation*}
H_{R}(g)(x')=\int_{R\le\rho(x',y)} K(x',y) (g-g\mathbf{1}_{B(x,\frac {R} 2)})(y) \,d\mu(y)
\end{equation*}
...
We  have
\begin{equation*}
(H_{r}-H_{R})(g-g\mathbf{1}_{B(x,\frac {R} 2)})(x')
\end{equation*}
\begin{equation*}
...
$\frac {R}{4}\le\rho(x',y)$, we can estimate \eqref{eqcotlar2} with \eqref{eqkernel-size} by
\begin{align*}
...
\label{second-cotlar-exception}
    |H_{r}(g\mathbf{1}_{B(x,\frac {R} 2)})(x')| > 2^{a^3 + 20a + 2} Mg(x)
\end{equation}
is less than or equal to  $\mu(B(x,\frac{R}{4}))/4$.
\end{lemma}
...
If $M(H_{r}g)(x)=0$, then $H_{r}g$ is zero almost everywhere and the estimate on $|F_1|$ is trivial.
Assume $M(H_{r}g)(x)>0$.
We have with \eqref{first-cotlar-exception}
\begin{equation}
    M(H_{r}g)(x)\ge
    \frac 1{\mu(B(x,\frac{R}{4}))}\int_{B(x,\frac{R}{4})}|H_{r}g(x')|\, dx'
\end{equation}
\begin{equation}
    \ge
    \frac 1{\mu(B(x,\frac{R}{4}))}\int_{F_1} 4 M(H_{r}g)(x)\, dx'
\end{equation}
...
|H_{R}g(x)| \le 2^{2}M(H_rg)(x)+ 2^{a^3+20a+3} Mg(x)
\, .
\end{equation}
\end{lemma}
...
by
\begin{equation}
    4 M(H_rg)(x) + 2^{a^3 + 20a + 2} Mg(x) + 2^{a^3 + 4a + 1} Mg(x)\, .
\end{equation}
This proves the lemma.
\end{proof}
...
\begin{equation}\label{eq-simple--nontangential}
    T_{r} g(x):=\sup_{r<R}\sup_{x'\in B(x,R)} |H_{R}(g)(x')| \, .
\end{equation}
\end{lemma}
\begin{proof}
...
\begin{equation}
     T_{r} g(x)
     \le 2^{a^3 + 2a + 2} Mg(x)+\sup_{r<R} |H_{R}(g)(x)|\, .
\end{equation}
...
In order to pass from the one-sided truncation in $H_r$ and $T_r$ to the two-sided truncation in $T_*$, we show in the following two lemmas that the integral in \eqref{def-tang-unm-op} can be exchanged for an integral over the difference of two balls.
\begin{lemma} \label{small-annulus}
    Let $f$ be a bounded measurable function with bounded support on $X$.
    Let $x\in X$ and $R>0$.
...
\end{lemma}
\begin{proof}
    %fixme: Do we know that X is nonempty? - yes, since \mu is non-zero :)
