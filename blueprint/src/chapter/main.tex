...
Let $(X,\rho,\mu,a)$ be a doubling metric measure space. Let $K$ be a two-sided Calder\'on--Zygmund kernel on $(X,\rho,\mu,a)$, i.e. a one-sided Calder\'on--Zygmund kernel
...
For $r > 0$, $x\in X$ and a bounded, measurable function $g$ on $X$ with bounded support, we define
\begin{equation}
\label{def-H_r}
H_r g(x):= \int_{r<|x-y|}
g(y) K(x,y) \, dy.
...
\uses{simple-nontangential-Hilbert}
    For every  bounded measurable function $g : X \to \C$ with bounded support we have
...
\begin{lemma}[Weak 1 1]
    ...
    Let $f:X\to\C$ be a bounded measurable function on with bounded support and assume that
    \begin{equation}
        \label{eq-strong-2-2-assumption}
        \|H_rf\|_{2}\leq 2^{a^3} \|f\|_2.
    \end{equation}
    . Let $\alpha>0$. Then for all $r>0$, we have
    \begin{equation}
        \label{eq-weak-1-1}
