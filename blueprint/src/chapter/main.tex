...
\begin{theorem}[two-sided metric space Carleson]
    ...
    \uses{metric-space-Carleson}
        ...
        \label{two-sided-resweak}
            \left|\int_{G} T f \, \mathrm{d}\mu\right| \leq \frac{2^{452a^3}}{(q-1)^5} \mu(G)^{1-\frac{1}{q}} \mu(F)^{\frac{1}{q}}\, .
        \end{equation}
\end{theorem}
...
\begin{lemma}[nontangential-from-simple]\label{nontangential-from-simple}
    \uses{simple-nontangential-operator}
...
\begin{proof}[Proof of \Cref{two-sided-metric-space-Carleson}]
    ...
    By the assumption \eqref{two-sided-Hr-bound-assumption}, we can apply \Cref{nontangential-from-simple} to obtain
    ...
    With \eqref{original-operator-assumption}, we obtain
...
\begin{lemma}
    ...
    %fixme: Rename a to something else, make the estimate sharp?
    For all real numbers $a\ge 4$,
    \begin{equation*}
...
%fixme: Break into smaller parts?
...
\begin{equation*}
    \mu\left(\{x\in X\setminus\Omega: |T_r b(x)|>{\alpha}/2\}\right)\,.
\end{equation*}
...
\begin{equation}
    ...
    \mu({\{x\in X\setminus\Omega: |T_r b(x)|>\alpha/2\}}) \le  \frac{2^{a^3+9a+4}}{\alpha} \int |f(y)|\,d\mu(y)\,.
\end{equation}
...
\begin{equation*}
    \mu\left(\{x\in X: |T_r f(x)|>\alpha\}\right) \le \frac{2^{2a^3+3a+2}c + \frac{2^{5a}}{c} + 2^{a^3+9a+4}}{\alpha} \int |f(y)|\, d\mu(y).
\end{equation*}
...
\begin{theorem}[two-sided metric space Carleson]
    \label{two-sided-metric-space-Carleson}
    \uses{metric-space-Carleson,non-tangential-from-simple}
        For all  integers $a \ge  4$ and real numbers $1<q\le 2$
        the following holds.
        Let $(X,\rho,\mu,a)$ be a doubling metric measure space. Let  $\Mf$ be a
        cancellative compatible  collection of functions and let $K$ be a two-sided Calder\'on--Zygmund kernel on $(X,\rho,\mu,a)$. Assume  that for every bounded measurable function $g$ on $X$ supported on a set of finite measure and all $r>0$ we have
...
This latter proof refers to the two-sided variant of the Carleson \Cref{two-sided-metric-space-Carleson}. Two assumptions in \Cref{metric-space-Carleson} require more work. The boundedness of the operator $T_r$ defined in \eqref{def-T_r} is established in \ref{Hilbert-strong-2-2}. This lemma is proved in \Cref{10hilbert}.
...
\uses{two-sided-metric-space-Carleson,real-line-metric,real-line-measure,real-line-doubling,oscillation-control,frequency-monotone,frequency-ball-doubling,frequency-ball-growth,integer-ball-cover,real-van-der-Corput,Hilbert-strong-2-2,Hilbert-kernel-regularity,Hilbert-kernel-bound}
\leanok
\lean{rcarleson}
...
One of the main assumptions of \Cref{two-sided-metric-space-Carleson}, concerning the operator $T_r$ defined in \eqref{def-T_r}, is verified by the following lemma, which is proved in \Cref{10hilbert}.
...
\begin{lemma}[Dirichlet kernel - Hilbert kernel relation]
    ...
    \uses{Dirichlet-kernel,lower-secant-bound}
    For all $N\in\Z$ and $z\in [-\pi,\pi] \setminus \{0\}$,
    \begin{equation*}
        \left|K_N(z) - (e^{-iNz}\kappa(z) + \overline{e^{-iNz}\kappa(z)})\right| \le \pi \,.
    \end{equation*}
    %fixme: Add something like "for a bounded function \tilde{K}_N(z) ?
\end{lemma}

\begin{proof}
    %fixme: replace z by x or y?
    Let $N\in\Z$ and $z\in [-\pi,\pi] \setminus \{0\}$. With \Cref{Dirichlet-kernel}, we obtain
    \begin{equation*}
        K_N(z) - (e^{-iNz}\kappa(z) + \overline{e^{-iNz}\kappa(z)})
        = e^{-iNz} \frac{\min(|z|, 1) }{1 - e^{iz}} + e^{iNz} \frac{\min(|z|, 1) }{1 - e^{-iz}} \,.
    \end{equation*}
    Using \Cref{lower-secant-bound} with $\eta = \min(|z|, 1)$, we bound
    \begin{equation*}
        \left|K_N(z) - (e^{-iNz}\kappa(z) + \overline{e^{-iNz}\kappa(z)})\right|
        \le \frac{\min(|z|, 1) }{|1 - e^{iz}|} + \frac{\min(|z|, 1)}{|1 - e^{-iz}|}
...
\begin{lemma}[partial Fourier sum bound]
    ...
    %fixme: avoid explicit \delta here?
...
\begin{proof}
    ...
    \begin{equation*}
        \frac{1}{2\pi} \int_{x-\pi}^{x+\pi} |g(y)| \left|K_N(z) - e^{-iN(x-y)}\kappa(x-y) + \overline{e^{-iN(x-y)}\kappa(x-y)}\right|\, dy
        \le \pi\delta \,.
    \end{equation*}
...
%fixme: Put this somewhere else and complete statement and proof
\begin{lemma}[real Carleson operator measurable]
    \label{real-Carleson-operator-measurable}
    $Tf$ as defined in ... is measurable.
\end{lemma}
\begin{proof}
    \leanok
\end{proof}
...
\begin{lemma}[partial Fourier sums of small]
    ...
    \begin{equation} \label{g-small}
        |g(x)|\le \delta \,.
    \end{equation}
    Then for every $\epsilon>0$, there exists a measurable set $E\subset [0,2\pi]$ such that for every $x\in [0,2\pi]\setminus E$ and $N>0$,
    \begin{equation}  \label{S_Ng-small}
...
\end{equation*}
Then \eqref{S_Ng-small} clearly holds, and it remains to show that $|E| \le \epsilon$.
