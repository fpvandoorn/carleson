...
\label{S-truncation}
\uses{-Hardy-Littlewood,ball-metric-entropy,kernel-summand,finitary-S-truncation}
...
\uses{counting-balls,cover-big-ball,boundary-measure}     There exists a grid structure $(\mathcal{D}, c,s)$.
\end{lemma}
...
\end{equation}
\lars{This used to say $2\fp \lesssim 100\mathfrak{m}$, but I have no idea why}
and
...
\label{forest-union}
\uses{forest-operator,C-dens1,C6-forest, forest-geometry, forest-convex,forest-separation, forest-inner}
  Let
  \begin{equation}
...
\begin{equation}\label{ubymsum}
     \mathbf{1}_{\sc(\fu)}(x)
    \le 2^{-j}\sum_{\mathfrak{m} \in \mathfrak{M}(k,n): 2\fu\lesssim 100 \mathfrak{m}} \mathbf{1}_{\sc(\mathfrak{m})}(x)\, .
\end{equation}
...
\begin{proof}
    ...
    \end{equation}
    Now suppose that $B_{\fu}(\fcc(\fu), 100) \cap B_{\fu'}(\fcc(\fu'), 100) = \emptyset$. Then we have $\mathfrak{B}(\fu) \cap \mathfrak{B}(\fu') = \emptyset$, by the definition \eqref{defbfp} of $\mathfrak{B}$ and the definition \eqref{wiggleorder} of $\lesssim$, but also $\mathfrak{B}(\fu) \subset \mathfrak{B}(\fp)$ and $\mathfrak{B}(\fu') \subset \mathfrak{B}(\fp)$, by \eqref{defbfp}, \eqref{wiggleorder} and \eqref{eq-Fefferman-trick0} \asgar{can't get neither of the inclusions $\mathfrak{B}(\fu) \subset \mathfrak{B}(\fp)$ and $\mathfrak{B}(\fu') \subset \mathfrak{B}(\fp)$ just from \eqref{defbfp}, \eqref{wiggleorder} and \eqref{eq-Fefferman-trick0} or Lemmas \ref{wiggle-order-1}, \ref{wiggle-order-2}, \ref{wiggle-order-3}}. \lars{The definition of $\mathfrak{B}(\fp)$ should have been the one for which this is just transititvity of $\lesssim$, I changed it now. I don't know why it was different before. The exact definition is only used here, so this does not cause problems elsewhere}
    Hence,
    $$
...
\begin{proof}
    Suppose that $\fp, \fp'' \in \mathfrak{T}_2(\fu)$. Then by \Cref{C5convex}, we have
...
\begin{proof}[Proof of \ref{correlation-separated-trees}]
    ...
    \begin{lemma}[correlation near tree parts]
        ...
        \uses{tree-projection-estimate,adjoint-tile-support,dyadic-partition-2,bound-for-tree-projection}
        We have for all $\fu_1 \ne \fu_2 \in \fU$ with $\sc(\fu_1) \subset \sc(\fu_2)$ and all bounded $g_1, g_2$ with bounded support
        \begin{equation}
...
\subsection{Hölder estimates for adjoint tree operators}
\label{subsubsec-holder-estimates}
    ...
    \begin{lemma}[Holder correlation tree]
        ...
        \uses{adjoint-tile-support,global-tree-control-1,global-tree-control-2}
    ...
    \begin{lemma}[global tree control 1]
        ...
        \uses{adjoint-tile-support,Holder-correlation-tile,scales-impacting-interval}
...
\begin{lemma}[row correlation]
    ...
    \uses{adjoint-tile-support,adjoint-tree-control,correlation-separated-trees}
...
\uses{metric-space-Carleson,real-line-metric,real-line-measure,real-line-doubling,frequency-ball-doubling,frequency-ball-growth,integer-ball-cover,real-van-der-Corput,Hilbert-kernel-bound,Hilbert-kernel-regularity}
...
\end{equation}
\end{lemma}
The following lemma is proved in Subsection \ref{10vandercorput}. \lars{The change to $\alpha$ and $\beta$ is not implemented everywhere yet.}
...
\begin{lemma}
   \label{CZD}
   Let $f$ be a piecewise continuous, bounded function with bounded support on $\mathbb{R}$. Let $\alpha>0$. Then there exists sets $A\subset \mathbb{R}$ such that the following properties \eqref{eq-CZ-good-set}, \eqref{eq-CZ-bad-sets-1} and \eqref{eq-CZ-bad-sets-2} are satisfied.
    We have for all $x\in A$
    \begin{equation}
    \label{eq-CZ-good-set}
        |f(x)|\leq \alpha.
    \end{equation}
    The set $\mathbb{R}\setminus A$ can be decomposed into the union of intervals
    $$\mathbb{R}\setminus A= \cup_{j=1}^{\infty} [a_j, b_j],$$ so that
   \begin{equation}
   \label{eq-CZ-bad-sets-1}
    (a_j, b_j)\cap (a_{j'}, b_{j'})=\emptyset \text{ for } j\neq j' \,
   \end{equation}
   and for each $j$
   \begin{equation}
       \label{eq-CZ-bad-sets-2}
       \alpha\leq \frac{1}{b_j-a_j}\int_{a_j}^{b_j} |f(y)|\, dy\leq 2^{4} \alpha.
   \end{equation}
\end{lemma}
...
Observe that
\begin{equation*}
T_*^{r_0}g(x)=\sup_{r_0<r_1<r_2<1}\sup_{|x-x'|<r_1}\left|H_{r_1} g(x')-H_{r_2} g(x')\right|\,,
\end{equation*}
so that
\begin{equation}
    \label{eq-1-sup}
    T_*^{r_0}g(x)\leq 2\sup_{r_0<r<1} T_r g(x).
\end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \|H_rg\|_2 = \|H_r f\|_2 \le 2^{16} \|f\|_2 = \|g\|_2\,.
    \end{equation}
...
|H_rg(x)-H_rg(x')|\leq 2^{14} Mg(x)\, .
\end{equation}
\end{lemma}

\begin{proof}
We have using triangle inequality that
\begin{equation}
    |H_rg(x)-H_rg(x')|
    \end{equation}
    \begin{equation}
    \label{eq-HrtoH2r}
    \leq |H_{2r}g(x)-H_{r}g(x)|+|H_{2r}g(x)-H_{2r}g(x')|+|H_{2r}g(x')-H_rg(x')|.
\end{equation}
...
\begin{equation}
    \label{eq-H2rx}
    |H_{2r}g(x)-H_{r}g(x)|\leq 16 Mg(x).
\end{equation}

Arguing similarly, for the last term in \eqref{eq-HrtoH2r}, we have
\begin{equation*}
    |H_{2r}g(x')-H_rg(x')|\leq \frac{8}{r} \int_{|x'-y|<2r}|g(y)|\, dy.
\end{equation*}
Since $|x-x'|<r$, for any $y$ with $|x-y|<2r$, triangle inequality yields
$$|x-y|<r+2r<3r.$$
Therefore by increasing the domain of integration, we can estimate
\begin{equation}
    \label{eq-H2rx'}
    |H_{2r}g(x')-H_rg(x')|\leq \frac{8}{r} \int_{|x-y|<3r}|g(y)|\, dy\leq 32 Mg(x).
\end{equation}
Finally, we estimate the middle term in \eqref{eq-HrtoH2r}. We have
$$|H_{2r}g(x)-H_{2r}g(x')|$$ $$\leq  \int_{\{y:\, 2r<|x-y|<1\}\cup \{y:\, 2r<|x'-y|<1\}} |\kappa(x-y)-\kappa(x'-y)||g(y)|\, dy.$$
For $y$ with $|x'-y|>2r$, we have
$$|x-y|\geq |x'-y|-|x-x'|>2r-r=r.$$
Therefore the domain of integration above is contained in the set
$\{y:\, |x-y|<r\}.$ Decomposing this set into dyadic ranges, we see that the integral above is estimated by
\begin{equation}
...
To see the above, let $z=x-y$ and $z'=x-y'$. Since $|z-z'|<r<\frac{|z|}{2}$, we conclude that $z, z'$ have the same sign. Since $\kappa(z)=\overline{\kappa(-z)}$, we may assume without loss of generality that they are both positive. Then our assumption implies that
    $$
        \frac{z}{2} \le z' \,.
    $$
...
Plugging the above into \eqref{eq-rker-HL} gives
\begin{equation}
    \label{eq-H2r}
    |H_{2r}g(x)-H_{2r}g(x')|\leq 2^{11} Mg(x) \sum_{j=0}^{\infty} 2^{-j}\leq 2^{12} Mg(x).
\end{equation}
\eqref{eq-non-tang-diff} now follows by combining \eqref{eq-HrtoH2r}, \eqref{eq-H2rx}, \eqref{eq-H2rx'} and \eqref{eq-H2r}.
\end{proof}
...
\begin{lemma}
\label{lem-Tr-to-T}
Let $0<r_0<r<1$ and  $x\in \mathbb{R}$. Let $g$ be a bounded measurable function on $\mathbb{R}$ with bounded support. Then, for all $x'\in \mathbb{R}$, we have
\begin{multline}
\label{Tr-est1}
|T_rg(x)|\leq  |H_{r_0}g_{x,2}(x)-H_{r_0}g_{x,2}(x')|+|H_{r_0} g(x')|+|H_{r_0} g_{x,1}(x')|+2^{12} Mg(x)\,,
...
\begin{equation*}
    \label{gg2}
    H_r g(x)= H_{r_0}g_{x,2}(x).
\end{equation*}
Using the triangle inequality twice, we estimate
\begin{equation*}
\label{eq-diff-xx'}
|H_{r_0}g_{x,2}(x)|\leq |H_{r_0}g_{x,2}(x)-H_{r_0}g_{x,2}(x')|+|H_{r_0}g_{x,1}(x')| + |H_{r_0}g(x')|.
\end{equation*}
...
\begin{lemma}
    \label{lem-cotlar}
    Let $0<r_0<r<1$ and $x\in \mathbb{R}$. Let $g$ be a bounded, measurable function with bounded support on $\mathbb{R}$. Then, there exists $x'$ with $|x-x'|<\frac{r}{2}$ such that
such that
\begin{equation}
    \label{eq-cotlar}
    |H_{r_0}g_{x,1}(x')|+|H_{r_0}g(x')|\leq  2^{12} [(M(|H_{r_0}g|^{\frac{3}{2}})(x))^{\frac{2}{3}}+(M(|g|^2)(x))^{\frac{1}{2}}],
\end{equation}
where $(M(|H_{r_0}g|^{\frac{3}{2}})$ and $M(|g|^2)$ are measurable functions satisfying \rs{The second one will only satisfy a weak (2, 2) type inequality. To be changed after writing CZ subsection}
\end{lemma}
\begin{proof}
For $\alpha>0$, we have
\begin{equation}
...
$$\|M(|H_{r_0}g|^{\frac{3}{2}})^{\frac{2}{3}}\|_2\leq 2^{16}\left(\frac{2}{2-\frac{3}{2}}\right)\|H_{r_0}g\|_2\leq 2^{20} \|H_{r_0}g\|_2.$$
Similarly, we have
\begin{equation*}
...
\begin{equation*}
    ...
    \alpha=\max\left(4 (M(|H_{r_0}g|^{\frac{3}{2}})(x))^{\frac{2}{3}}, 2^{12} (M(|g|^2)(x))^{\frac{1}{2}} \right),
\end{equation*}
we have the estimate
\begin{multline*}
    \mu(\{x'\in \mathbb{R}: |x-x'|<\frac{r}{2}, |H_{r_0}g_{x,1}(x')|>\alpha \text{ or }|H_{r_0}g(x')|>\alpha\}\\\leq \frac{1}{2} \mu(\{x'\in \mathbb{R}: |x-x'|<\frac{r}{2}\}.
\end{multline*}
...
\begin{equation}
\label{eq-xx'-g2-diff}
|H_{r_0}g_{x,2}(x)-H_{r_0}g_{x,2}(x')|\leq 2^{11} Mg(x)\, .
\end{equation}
\end{lemma}
\begin{proof}
We have
$$|H_{r_0}g_{x,2}(x)-H_{r_0}g_{x,2}(x')|\leq \int_{r<|x-y|<1} |\kappa(x-y)-\kappa(x'-y)||g(y)|\, dy.$$
...
\begin{equation*}
 |H_{r_0}g_{x,2}(x)-H_{r_0}g_{x,2}(x')|\leq 2^{10} \sum_{j=0}^{\lfloor \log_2(\frac{1}{r})\rfloor}2^{-j}Mg(x)\leq 2^{11} Mg(x).
\end{equation*}
\end{proof}
