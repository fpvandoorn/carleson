...
\begin{proof}[Proof of \Cref{local-dens1-tree-bound}]
    ...
    $$
        d_{\fp'}(\fcc(\fp'), q) \le d_{\fp'}(\fcc(\fp'), \fcc(\fq)) + d_{\fq}(\fcc(\fq), q) \le 9\,.
    $$
    Thus $L \cap E(\fq) \subset E_2(9, \fp')$. Since $\scI(\fp'') \subset \scI(\fp') \subset \scI(\fp)$ and $\fp'', \fp \in \fT(\fu)$, we have $\fp' \in \fP(\fT(\fu))$. We deduce using the definition \eqref{definedens1} of $\dens_1$
    $$
        \mu(L \cap \bigcup_{\fq \in \fT(\fu)} E(\fq)) \le \mu(E_2(9, \fp')) \le 9^a \dens_1(\fT(\fu)) \mu(L')\,.
    $$
...
\label{subsubsec-holder-estimates}
    ...
    \begin{lemma}[global tree control 1]
        ...
        \leanok
        \lean{TileStructure.Forest.global_tree_control1_1, TileStructure.Forest.global_tree_control1_2, TileStructure.Forest.global_tree_control1_3}
...
\leanok
\lean{simple_nontangential_operator}
